...
git add AndroidGarage/screenshots/store/
```
...
The pipeline also regenerates the **Play Store** asset set in `AndroidGarage/screenshots/store/` (phone + 7"/10" tablet screenshots, via `scripts/generate-store-screenshots.py` wired in after the framing step). That dir is committed staging — promoting the curated subset into `AndroidGarage/distribution/playstore/` and uploading to Play Console is a separate manual step. See the **`play-store-assets`** skill.
