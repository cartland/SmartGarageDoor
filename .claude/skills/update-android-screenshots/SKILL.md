---
description: Update Android reference screenshots for image tracking (not pixel-perfect verification).
allowed-tools: Bash(*), Read, Glob, Grep
user-invocable: true
---

# Update Android Screenshots

Generate reference screenshots for tracking app appearance over time. These are for documentation and visual history — NOT pixel-perfect CI verification. Screenshots never block CI or PRs.

## Purpose

- Track how the app looks across releases
- Generate Play Store assets and documentation images
- Provide visual history of UI changes in git
- Light/dark theme variants for all key screens and components

## Steps

### 1. Generate screenshots sequentially (avoids OOM)

```bash
./scripts/generate-android-screenshots.sh
```

This runs each test file in its own Gradle invocation, then auto-generates the gallery.

### 2. Review the gallery

Open `MobileGarage/android-screenshot-tests/SCREENSHOT_GALLERY.md` to see all screenshots.

### 3. Commit the references

```bash
git add MobileGarage/android-screenshot-tests/src/screenshotTestDebug/reference/
git add MobileGarage/android-screenshot-tests/SCREENSHOT_GALLERY.md
git add MobileGarage/screenshots/framed/
git add MobileGarage/screenshots/store/
```

The framed screenshots in `MobileGarage/screenshots/framed/` are auto-generated by `scripts/frame-screenshot.py` from a curated allowlist (`scripts/framed-screenshots.txt`). They wrap selected raw references in a programmatic Pixel-style bezel for README / docs use. To add or remove a framed output, edit the allowlist; raw references stay untouched. The script also emits `MobileGarage/screenshots/framed/COLLECTION.md` — an auto-regenerated index showing all framed shots grouped by view with light + dark side by side.

The pipeline also regenerates the **Play Store** asset set in `MobileGarage/screenshots/store/` (phone + 7"/10" tablet screenshots, via `scripts/generate-store-screenshots.py` wired in after the framing step). That dir is committed staging — promoting the curated subset into `MobileGarage/distribution/playstore/` and uploading to Play Console is a separate manual step. See the **`play-store-assets`** skill.

Framing requires Pillow (`pip install Pillow`). If Pillow is missing, the framing step prints a warning and is skipped — the rest of the pipeline still runs.

**Before adding a preview to the allowlist, check `MobileGarage/docs/SCREENSHOT_FRAMING_PLAN.md`** — it tracks the goal state for the framed set and lists shipped issues with PR refs.

## Adding New Screenshots

1. Create or find a `@Preview` composable in the app code
2. Add a screenshot test in `android-screenshot-tests/src/screenshotTest/kotlin/.../`:

```kotlin
@PreviewTest
@Preview(showBackground = true, name = "Light")
@Preview(showBackground = true, name = "Dark",
    uiMode = android.content.res.Configuration.UI_MODE_NIGHT_YES)
@Composable
fun MyComponentPreviewTest() {
    AppTheme {
        MyComponentPreview()
    }
}
```

3. Run `./scripts/generate-android-screenshots.sh` to generate the reference PNGs

## Cleanup

The script automatically cleans old reference PNGs before generating new ones — outdated screenshots are removed, not accumulated. The gallery is regenerated from scratch each run, so orphaned entries are impossible.

## Cross-Platform Consistency

Screenshots render differently across platforms (macOS vs Linux CI). To avoid thrashing:
- Only regenerate screenshots deliberately via this skill or the script
- CI compiles screenshot tests but never generates or validates PNGs
- Commit screenshots from a single platform consistently

## Notes

- `updateDebugScreenshotTest` and `validateDebugScreenshotTest` can't run in the same Gradle invocation
- To force single-invocation: `./gradlew :android-screenshot-tests:updateDebugScreenshotTest -PforceAllScreenshots`
- Preview composables must be deterministic — use fixed `Instant.parse(...)` for timestamps, never `Clock.System.now()` or `Random`
- Animated composables (`rememberInfiniteTransition`, `Animatable`, `animateFloatAsState`) render at an arbitrary frame in screenshot tests. Wrap previews of animated UI in a static-mode rendering path — e.g. `GarageIcon(static = true)` — so the screenshot is deterministic. Caught when `OpeningPreview` was rendering at the start frame (offset 0.0) and looked identical to `Closed`; PR #543 fixed it by routing previews through `static = true` so they share the recent events list's `staticPositionFor` mapping.
- Reference PNGs are committed to git but are NOT validated in CI
- The gallery markdown is auto-generated — do not edit it manually
- **`rememberAppComponent()` crashes Layoutlib.** The screenshot-test plugin's `ApplicationContext` cannot be cast to `GarageApplication`, so any Composable that calls `rememberAppComponent()` (directly or transitively) throws `ClassCastException` and the entire preview collapses to 1×1. Previews must NOT reach the DI graph. Fix pattern: hoist the DI lookups up into the screen-level Composable (next to where ViewModels and `LocalContext` are already resolved) and pass state to inner Composables as parameters. Each card/component should have a single stateless implementation. PR #564 used this pattern to fix `SettingsTabPreviewTest` — `UserInfoCard` and `AndroidAppInfoCard` had stateful overloads calling `rememberAppComponent` that were dead after hoisting and got deleted.
- **`painterResource(R.drawable.X)` is broken in `com.android.compose.screenshot:0.0.1-alpha12`.** The plugin's renderer throws `IllegalArgumentException: Only VectorDrawables and rasterized asset types are supported` even on valid VectorDrawables (investigated in PRs #560-#563 — replacing path data, copying known-good drawable contents, stripping CRLF: all still fail). Prefer Compose Material `Icons.Filled.*` (pure `ImageVector` — bypasses the resource loader) for new icons. PR #563 converged production+previews on `Icons.Filled.Timer` + `Icons.Filled.CalendarMonth`, deleted the custom `clock_icon.xml` / `calendar_icon.xml` drawables, and removed the `PreviewSafeIcon` workaround.
