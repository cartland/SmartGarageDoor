...
- `AndroidGarage/iosApp/SnapshotTests/` — the snapshot test target (`GarageControlSnapshotTests`, a host-based unit-test bundle).
...
- `AndroidGarage/iosApp/.prefire.yml` — Prefire config (`target: GarageControl`, `imports: [shared]` so previews referencing Kotlin types compile in the generated tests).
