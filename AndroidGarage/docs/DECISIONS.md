...
- **The generated test file is NOT committed.** `PreviewTests.generated.swift` is fully derived from the `#Preview` macros + `.prefire.yml` + the Prefire version, so it is gitignored and regenerated before every run — no committed generated code, and it cannot drift from the previews. The committed artifacts are the **outputs**: the reference PNGs and `SCREENSHOT_GALLERY.md`. (On a fresh checkout the snapshot test target therefore has no Swift source until the script runs; the app build and gating iOS CI never compile it — it builds only for the test action — so nothing breaks.)
...
- `AndroidGarage/iosApp/SnapshotTests/PreviewTests.generated.swift` — generated by the `prefire` CLI; **gitignored**, regenerated by the script.
