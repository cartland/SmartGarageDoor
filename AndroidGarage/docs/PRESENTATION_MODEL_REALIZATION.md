...
- **Settings** — ~~3-tab restructure + gated Developer section (#926)~~;
  ...
  (denied → tap-to-enable row); ~~privacy-policy link~~ ✅ (the URL became a shared
  `AppLinks.PRIVACY_POLICY_URL` in `domain/commonMain` — single source of truth,
  Android's Settings + iOS both consume it). Remaining: App Store link (deferred
  until published).
