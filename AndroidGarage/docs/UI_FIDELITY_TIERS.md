...
|---|---|---|
| Door-status visualization (canvas geometry, panel layout, proportions) | **1** | Brand through-line. **Geometry + fill palette + the animation spec are now shared** — `:domain` `GarageDoorGeometry` + `GarageDoorPalette` + `DoorAnimation` (offset constants + `DoorPosition → offset/overlay/color-state` mappings), consumed by both canvases (pinned by `GarageDoorGeometryTest` / `GarageDoorPaletteTest` / `DoorAnimationTest`). Only the iOS live-trajectory convergence remains (PENDING_FOLLOWUPS § 4). |
