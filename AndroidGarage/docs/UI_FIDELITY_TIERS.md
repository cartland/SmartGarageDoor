...
|---|---|---|
| Door-status visualization (canvas geometry, panel layout, proportions) | **1** | Brand through-line. **Geometry + fill palette + the animation spec are now shared** — `:domain` `GarageDoorGeometry` + `GarageDoorPalette` + `DoorAnimation` (offset constants + `DoorPosition → offset/overlay` mappings), consumed by both canvases (pinned by `GarageDoorGeometryTest` / `GarageDoorPaletteTest` / `DoorAnimationTest`). Only the color-state mapping + the live-trajectory convergence remain (PENDING_FOLLOWUPS § 4). |
...
| Door **animation** trajectory + once-per-event replay | **1 (spec) / 3 (execution)** | Splits by layer: the offsets/targets/curve-as-function/replay *policy* are Tier-1 provable (hoist + `commonTest` + sampled-progress snapshots); the real-time interpolation is Tier-3 best-effort (native engine + reduce-motion). **Spec now shared** — `:domain` `DoorAnimation` (offsets/targets/static/overlay, pinned by `DoorAnimationTest`); iOS consumes the shared displayed-offset + overlay. iOS adopting the live trajectory (slide/spring/replay) is the remaining **(b)** convergence. See [§ Animation: split the spec from the execution](#animation-split-the-spec-provable-from-the-execution-best-effort) below + ADR-025. |
...
**State of this codebase.** Android's `DoorAnimation` (`AnimatableGarageDoor.kt`; contract in [`DOOR_ANIMATION.md`](./DOOR_ANIMATION.md)) is already a clean pure spec; iOS `DoorVisual` (`GarageDoorCanvas.swift`) is a *partial, divergent* re-implementation (motion states parked at the static mid-offsets under one 0.6 s ease; no `fromPositionFor` slide, no spring/tween split, no replay gate). So the pending offsets/mappings work is **two** things:
- **(a) Offsets + offset/overlay mappings — SHIPPED.** The 5 offset constants + the full pure `DoorAnimation` spec object (`targetPositionFor`/`fromPositionFor`/`useSpringFor`/`staticPositionFor`/`overlayFor`) + the `DoorOverlayKind` enum now live in `:domain` `DoorAnimation`, read by Android `GarageIcon` and iOS `GarageDoorView` (iOS consumes `staticPositionFor` + `overlayFor`; Android also drives its live trajectory from target/from/spring); pinned by `DoorAnimationTest` (commonTest, moved from the Android unit test). Verified pure no-op (mappings already identical; byte-identical iOS snapshots). The `colorState` mapping was carved out (its enum is woven through Android's theme package across 3 UI files) — a small separate follow-up.
- **(b) Trajectory convergence** — **not** a no-op: iOS *adopting* the shared spec (from→target slide, tween-vs-spring family, replay-once-per-event) changes iOS behavior, so verify it with layer 2 (spec pin + sampled-progress static snapshots) and leave the real-time interpolation as honest Tier-3 best-effort. Decide first whether the 12 s garage-door-speed pacing is brand-essential (Tier 1 → make iOS match) or acceptable native taste (Tier 3 → leave iOS's ease): the slide *direction / targets / overlay* are brand; the exact pacing is closer to Tier-3.
