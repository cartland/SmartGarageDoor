...
3. **Phase F — release tooling + first TestFlight** (1–2 PRs):
   ...
   - ⏳ **`scripts/release-ios.sh` + the archive/upload workflow — NOW UNBLOCKED (signing done 2026-06-30), not yet built.** Mirror `release-android.sh` (bump `MARKETING_VERSION` + `CURRENT_PROJECT_VERSION`, tag `ios/N`) + a `release-ios.yml` doing `xcodebuild archive` + TestFlight upload (+ the `## N.M.K` changelog gate). Needs an **App Store Connect API key** as GitHub secrets — see [`../../docs/IOS_RELEASE_SETUP.md`](../../docs/IOS_RELEASE_SETUP.md) § "Future: automated releases".
