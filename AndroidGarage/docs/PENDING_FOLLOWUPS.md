...
- **Door constants shared-hoist (systemic).** The door geometry/palette/offsets/mappings were **hand-duplicated** Kotlin↔Swift with no guard — they matched by coincidence, so an edit on one side silently drifted the brand on the other. Hoist to shared `:domain` constants (the `AppLinks` pattern), consumed by both. Done in slices:
  ...
  - ⬜ **colorState mapping + live-trajectory convergence.** (i) `DoorPosition→colorState` (`doorColorState()` ↔ iOS `DoorVisual.colorState`) still mirrors — carved out of the hoist above because the `DoorColorState` enum is woven through Android's theme package + 3 UI consumers (a `typealias` would keep them zero-churn). (ii) iOS adopting Android's live trajectory (from→target slide, tween-vs-spring, replay-once-per-event, + a sampleable `offsetAt`) is the **(b)** convergence in [UI_FIDELITY_TIERS § Animation](./UI_FIDELITY_TIERS.md#animation-split-the-spec-provable-from-the-execution-best-effort) — **not** a no-op; verify via spec pin + sampled-progress snapshots.
