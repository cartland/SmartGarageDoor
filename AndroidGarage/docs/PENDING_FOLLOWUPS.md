...
3. **Phase F — release tooling + first TestFlight** (1–2 PRs):
   ...
   - ⏳ **`scripts/release-ios.sh` + the archive/upload workflow — DEFERRED to the signing step.** Mirror `release-android.sh` (bump `CFBundleShortVersionString` + `CFBundleVersion`, tag `ios/N`) + a `release-ios.yml` doing `xcodebuild archive` + TestFlight upload (+ the `## N.M.K` changelog gate). Held until Apple code-signing so it can be tested end-to-end against a real archive/upload — a tag-push script with no workflow listening would be an inert, untestable half-feature.
