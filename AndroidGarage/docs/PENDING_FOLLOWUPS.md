...
- **Door constants shared-hoist (systemic).** The door geometry/palette/offsets/mappings were **hand-duplicated** Kotlin↔Swift with no guard — they matched by coincidence, so an edit on one side silently drifted the brand on the other. Hoist to shared `:domain` constants (the `AppLinks` pattern), consumed by both. Done in slices:
  ...
  - ⬜ **Live-trajectory convergence (b).** iOS adopting Android's live trajectory (from→target slide, tween-vs-spring, replay-once-per-event, + a sampleable `offsetAt`) — the **(b)** convergence in [UI_FIDELITY_TIERS § Animation](./UI_FIDELITY_TIERS.md#animation-split-the-spec-provable-from-the-execution-best-effort). **Not** a no-op (real iOS behavior change); verify via spec pin + sampled-progress snapshots; decide first whether the 12 s garage-door pacing is brand-essential (Tier 1 → match) or acceptable native taste (Tier 3 → leave iOS's ease).
