...
- **Door constants shared-hoist (systemic).** The door geometry/palette/offsets/mappings were **hand-duplicated** Kotlin↔Swift with no guard — they matched by coincidence, so an edit on one side silently drifted the brand on the other. Hoist to shared `:domain` constants (the `AppLinks` pattern), consumed by both. Done in slices:
  ...
  - ⬜ **Color palette** — the 12 light/dark door-state hex pairs (`Color.kt` ↔ iOS `DoorPalette`). Entangled with the Android theme path (`DoorStatusColorScheme` + the `on*` text variants the canvas doesn't use); share the 6 fill colors as shared RGB-int constants.
  - ⬜ **Offsets + state-mappings** — the 5 offset positions + `DoorPosition→offset/overlay/colorState` (`AnimatableGarageDoor.kt` + `DoorStatusColorScheme.kt` ↔ iOS `DoorVisual`). Touches Android's animation API (`targetPositionFor`/`fromPositionFor`/`useSpringFor` stay Android-only — animation is deferred); share the static-offset + overlay + colorState mappings via shared enums + a pinning test.
