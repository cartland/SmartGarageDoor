...
re-runs the `GarageControlSnapshotTests` target on a simulator — the target's pre-build
...
- A `#Preview` body is embedded **verbatim** into the generated `PreviewTests`,
  which is in the test target (`@testable import GarageControl`). So the body may only
