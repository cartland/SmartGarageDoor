...
(`./scripts/generate-android-screenshots.sh`) + `AndroidGarage/distribution/playstore/generate.sh`.
These are the **candidates**; the curated subset that's live in the store
is copied by hand into `AndroidGarage/distribution/playstore/` (see the `play-store-assets` skill).
