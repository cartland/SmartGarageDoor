...
| `Error: The following functions are found in your project but do not exist in your local source code: <name>(us-central1)` followed by `Aborting because deletion cannot proceed in non-interactive mode` | A previous PR removed a function export from `src/index.ts` (or stopped registering it under `process.env.FUNCTION_NAME`). The function still exists in GCP, but the source no longer declares it — `firebase deploy` refuses to auto-delete in CI's non-interactive mode. | Delete the orphan manually from a real terminal, then re-run the deploy: `firebase functions:delete <name> --region us-central1 --project escape-echo --force`. The `--force` is required when the call is wrapped (`/exit` from a slash-command shell, CI, etc.) — without it the confirmation prompt aborts (you'll see `Error: Command aborted.`). After deletion succeeds, `gh run rerun <id> --failed` to re-trigger the original deploy. **First seen 2026-05-23 on the `server/27` deploy after PR #830 removed `httpNextEvent`** (the prior deploy `server/26` had still seen the function in source, so the orphan was new). Pattern recurs whenever a PR removes a function export. |
| `Request to https://cloudscheduler.googleapis.com/... had HTTP Error: 500, An internal error has occurred` on a `firebase-schedule-<pubsubFn>` job, then `⚠ functions: Deploys failed. Skipping deletes.` | Transient Google-side Cloud Scheduler error while updating a scheduled function's job — not a code or permissions problem. Some functions in the same deploy may have already updated successfully (deploys are per-function). | `gh run rerun <id> --failed` — the deploy is idempotent and the rerun brings every function to the tagged state. The self-verifying gate (greps for `Deploy complete!`, added #1048) is what surfaces this as a failed run instead of a silent partial deploy. **First seen 2026-07-05 on the `server/34` deploy** (`pubsubCheckForOpenDoorsJob`'s scheduler update 500'd after two other functions had updated); the rerun succeeded clean. |
