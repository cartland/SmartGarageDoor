---
pr: 270
state: merged
date: 2026-04-11
phase1_reviewed: 2026-04-20
---

# PR #270: ui: Make history cards 50% taller

**Goal:** Increase `GarageIcon` in history list items from 56dp → 84dp. Cards are ~50% taller, easier to read and tap. Updated screenshot references.

**Files:** `MobileGarage/androidApp/src/main/java/com/chriscartland/garage/ui/DoorStatusCard.kt`, `android-screenshot-tests/SCREENSHOT_GALLERY.md`, 6 screenshot PNGs.

## Phase 2 assessment

**Primary:** ok
**Secondary:** []
**Phase 2 reviewed:** 2026-04-20

**Rationale:** UI polish — still in place.
