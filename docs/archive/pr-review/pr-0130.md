---
pr: 130
state: merged
date: 2026-04-06
phase1_reviewed: 2026-04-20
---

# PR #130: feat: Move ServerConfigRepositoryImpl to data module (Phase 9)

**Goal:** First step of Phase 9. `ServerConfigRepositoryImpl` → `data/src/commonMain/kotlin/` (zero Android deps). Inject `serverConfigKey` via constructor instead of global `APP_CONFIG`. Remove duplicate `ServerConfigRepository` interface from `config/` (domain already has it). Update imports and DI wiring.

**Files:** `MobileGarage/data/src/commonMain/kotlin/.../ServerConfigRepositoryImpl.kt`, delete `androidApp/config/ServerConfigRepository.kt`, `AppComponent.kt`, `DoorRepositoryImpl.kt`, `PushRepository.kt`, test files.

## Phase 2 assessment

**Primary:** ok
**Secondary:** []
**Phase 2 reviewed:** 2026-04-20

**Rationale:** First repo moved to data/commonMain.
