---
pr: 380
state: merged
date: 2026-04-19
phase1_reviewed: 2026-04-20
---

# PR #380: ci: Skip Firebase CI on docs-only changes

**Goal:** Add a docs-only fast path to `firebase-ci.yml` — skip the Firebase unit-test job when every changed file matches the docs classification (markdown, `docs/**`, `.claude/**`, `LICENSE`, `.gitignore`, `MobileGarage/distribution/whatsnew/**`).

**Files:** `.github/workflows/firebase-ci.yml`.

## Phase 2 assessment

**Primary:** ok
**Secondary:** []
**Phase 2 reviewed:** 2026-04-20

**Rationale:** Parallel of #379 for Firebase; rule now in CLAUDE.md and memory, applied consistently.
