...
import { getDeveloperAuthorizedEmails } from '../../controller/config/ConfigAccessors';
...
/**
 * Pure core for the GET developer-access endpoint.
 *
 ...
 * users: this endpoint is a UI hint — the Android client uses it to
 * decide whether to render the Settings → Developer section. The actual
 * security boundary is the per-action handlers (Diagnostics is local-
 * only today, but any future server-touching dev affordance must
 * independently gate). Returning 403 here would force the client to
 * special-case "user is signed in but feature-denied," which is the
 * same affirmative answer as `{enabled: false}` — keep one path.
 */
export async function handleDeveloperAccess(input: {
  method: string;
  googleIdTokenHeader: string | undefined;
}): Promise<HandlerResult<DeveloperAccessResponse>> {
  if (input.method !== 'GET') {
  ...
  const allowed = getDeveloperAuthorizedEmails(config);
