...
- Token is `base64url({ v, bt, s, n, d })` (boundary Timestamp + direction, scoped to buildTimestamp); malformed/foreign tokens fall back leniently to a fresh first page. Shared wire-contract fixtures under `wire-contracts/eventHistory/` are loaded by both the server Mocha tests and the Android Ktor test. PR #TBD.
