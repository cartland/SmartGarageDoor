...
## server/35
- **New `doorCommand` endpoint — directional open/close, verdict-only, additive (no existing endpoint changes).** Takes `{"command":"open"|"close"}` and answers whether that command is actionable against the door's real state. Runs **in parallel to `addRemoteButtonCommand`** (the direction-less toggle the ESP32 polls) and does not replace it.
