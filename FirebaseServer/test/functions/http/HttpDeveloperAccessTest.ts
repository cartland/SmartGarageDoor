...
import { handleDeveloperAccess } from '../../../src/functions/http/DeveloperAccess';
import {
...
const FIXTURE_DIR = path.join(__dirname, '..', '..', '..', '..', 'wire-contracts', 'developerAccess');
...
describe('handleDeveloperAccess (pure handler core)', () => {
  ...
  beforeEach(() => {
    ...
    fakeConfig.seed({
      body: {
        featureDeveloperAllowedEmails: [ALLOWED_EMAIL],
      },
    });
  ...
  it('returns 405 Method Not Allowed for non-GET methods', async () => {
    const result = await handleDeveloperAccess(happyInput({ method: 'POST' }));
  ...
  it('returns 401 Unauthorized (token) when googleIdToken header is missing', async () => {
    const result = await handleDeveloperAccess(happyInput({ googleIdTokenHeader: undefined }));
  ...
  it('returns 401 Unauthorized (token) when googleIdToken header is empty string', async () => {
    const result = await handleDeveloperAccess(happyInput({ googleIdTokenHeader: '' }));
    expect(result).to.deep.equal({ kind: 'error', status: 401, body: { error: 'Unauthorized (token).' } });
  });

  it('returns 401 Unauthorized (token) when verifyIdToken throws', async () => {
    fakeAuth.failNextVerify(new Error('auth/invalid-id-token'));
    const result = await handleDeveloperAccess(happyInput());
    expect(result).to.deep.equal({ kind: 'error', status: 401, body: { error: 'Unauthorized (token).' } });
  });
  ...
  it('returns 200 enabled=false when allowlist is empty array', async () => {
    fakeConfig.clear();
    fakeConfig.seed({ body: { featureDeveloperAllowedEmails: [] } });
    const result = await handleDeveloperAccess(happyInput());
    expect(result).to.deep.equal({ kind: 'ok', data: ENABLED_FALSE_FIXTURE });
  });
