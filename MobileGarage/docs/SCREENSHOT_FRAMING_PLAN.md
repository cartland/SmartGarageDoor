...
Until 2026-04-25 the README's screenshots (`MobileGarage/screenshots/home_closed.png`, `history.png`, `user.png`) were one-off real-device captures. They were checked in once, never refreshed, and silently drifted from the live app as the UI changed (~4 months stale by 2026-04-25).

The screenshot test pipeline already produces fresh `@Preview` PNGs on every `./scripts/generate-android-screenshots.sh` run. The framing step layers a bezel + status bar onto a curated subset of those PNGs and writes them to `MobileGarage/screenshots/framed/`. The eventual goal is for the READMEs to point at the framed copies so they auto-update with the app.
...
sips -Z 2000 MobileGarage/screenshots/framed/*.png --out /tmp/framed-review/
```
