...
4. **`android/246` / 2.16.36 — Snooze save failures are now visible (Snackbar surface) + typed error for the door-state-change case.** To verify on a real device:
   - **`SnoozeAction.Failed.EventChanged` — door state changed during sheet open**: trigger an `OPENING` or `CLOSING` state on the door, open Settings → tap the Snooze row, pick any duration (1h is fine). While the sheet is still open OR within a few seconds of tapping Save, the door transitions to the next state (`Opening → OpeningTooLong` fires after ~60s automatically). Save the snooze. Expected: a snackbar appears at the bottom with **"Door state changed before snooze could apply. Try again."** (NOT a generic "Snooze did not apply" copy). Reproduces the original bug class from 2026-05-28 ("snooze didn't hold during stuck-opening"). The hardest cell to reproduce reliably — the timing requires the door to transition during the sheet-open-to-Save window. Easier path: deliberately leave a door `Opening` (block the OPEN sensor briefly), wait ~60s for `OpeningTooLong` to fire, then open the snooze sheet — by the time you tap Save the cached `lastChangeTimeSeconds` (from the prior `Opening` event) is stale and the submit hits 404.
   - **`Failed.NetworkError` — generic network failure**: airplane mode → open snooze sheet → pick duration → Save. Spinner runs briefly, then the snackbar shows **"Snooze did not apply. Check your connection and try again."**
   - **`Failed.NotAuthenticated` — signed out**: sign out of the Google account, then open the snooze sheet. (Note: the Snooze row may be hidden / row tap may behave differently when signed out — verify the path is actually reachable. If snoozing is gated behind auth in the UI before reaching the sheet, this variant may be unreachable in practice; that's fine.)
   - **`Failed.MissingData` — no recent door event**: probably unreachable in normal app state (`currentDoorEvent.value?.lastChangeTimeSeconds` would have to be null). If you can force it (clear DB during dev), the snackbar should show **"No recent door event available. Try again in a moment."** Otherwise skip — the copy is wired up and the dispatch path is unit-tested.
   - **Spinner-then-snackbar sequence**: the Snooze row's leading-icon spinner runs during `Sending`, stops when the result lands, and the snackbar appears immediately after. There should be no "stuck spinner" state.
   - **Snackbar auto-dismisses** after the standard Material 3 duration. `scheduleActionReset()` returns the underlying `snoozeAction` to `Idle` after 10s; the snackbar is independent of that timeout (it's the SnackbarHost's own duration handling).
   - **No snackbar on success**: snoozing in a stable state (OPEN or CLOSED, door not transitioning) should succeed silently with the row's secondary text updating to the new end time — no snackbar.
5. **`android/245` / 2.16.35 — Door-animation trajectory audit (instrumented).** No user-facing change. To verify: run `./scripts/run-instrumented-tests.sh` (or the post-merge instrumented job already does this) and confirm `GarageDoorAnimationBehaviorTest` passes all 5 tests on the connected device — the matrix is then empirically verified. No on-screen smoke needed; the test IS the verification.
