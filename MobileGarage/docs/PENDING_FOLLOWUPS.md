...
4. **Phase G — App Store** (submission itself user-gated; prep largely DONE 2026-07-05):
   ...
   - ✅ **Listing copy + FULL screenshot set — STAGED (#1057, #1064)** in `MobileGarage/distribution/appstore/`: `LISTING.md` (copy + App Privacy answers + review notes + headless idb capture runbook) and 12 screenshots (Home/History/Settings × light/dark, iPhone 6.9" + iPad 13", exact ASC sizes).
   ...
   - **Remaining (user): the ASC session only — the complete click-by-click walkthrough with every value pre-filled (incl. the verified privacy-policy URL) is [`../distribution/appstore/SUBMISSION_STEPS.md`](../distribution/appstore/SUBMISSION_STEPS.md).** The same doc carries the two device verifications (sign-in tap-through + push delivery on the TestFlight build) as an optional companion section.
