...
- **Internal:** #1131 — `ObserveWatchAppStatusUseCase` caches the repository's cold polling flow via `stateIn(applicationScope, WhileSubscribed(5s), Unknown)` and returns `StateFlow`; `DefaultProfileViewModel` passes it through by reference (ADR-022) instead of mirroring it into a `MutableStateFlow` seeded `Unknown` plus an `init` collector. Provider is `@Singleton` / `@SharedSingleton` in both DI components with matching `abstract val` entry points and `assertSame` identity tests (`ComponentGraphTest`, `NativeComponentTest`). `WhileSubscribed` deviates from the `Eagerly` house default deliberately — this upstream is a 15s Play Services poll, not a cheap in-memory combine, so `Eagerly` would poll for the whole process lifetime; cross-navigation retention relies on `replayExpirationMillis` defaulting to `Long.MAX_VALUE` and is pinned by `ObserveWatchAppStatusUseCaseTest.retainsLastStatusForALaterCollector`. `FakeWearCompanionRepository` gained `useColdSource()` (the hot `MutableStateFlow` default cannot reproduce this bug class). `WatchAppStatus` added to `checkViewModelStateFlow`'s banned-mirror list. Patch; no capability change.

## 2.23.0

- **Voice control now operates the real garage door (Developer-flag users, signed in).** The Home tab's voice card is live: speak "open the garage door", let the 3-second cancel ring complete, and the command presses the real remote garage button — the same auth-gated press path as the manual button. Every safety gate carries over unchanged: only high-confidence commands arm, the door-state gate runs at arm time and again at commit (open only when closed, close only when open; moving, anomalous, or stale check-in states refuse), tapping the ring cancels and re-listens, and nothing commits off-screen. The "Simulated" pill is gone; a sent command now reads "The door should respond shortly". The Settings → Developer playground remains fully simulated.
- **Internal:** #1129 — `RemoteButtonVoiceCommandEnvironment` (real door state in, `PushRemoteButtonUseCase` press out; a fresh ack token per press tagged `-voice` for server-log correlation — the server treats the token as opaque, ack-equality only) replaces `ShadowVoiceCommandEnvironment` (deleted; `VoiceDoorStateMapper` extracted to its own file). End-to-end `HomeViewModelTest` coverage: a committed command presses the button with a voice-tagged token; refusals and signed-out commits never press. No DI change (the VM already had the push UseCase). Minor: new capability (voice can operate the door); whatsnew unchanged (invisible to non-flag users).
...
- **Internal:** #1071–#1080 — the status-cache program (`docs/STATUS_CACHE_PLAN.md`, ADR-034): DataStore-backed `StatusSnapshotStore` (new backup-excluded `status_cache.preferences_pb`), hydrate-then-revalidate in `NetworkButtonHealthRepository` / `NetworkSnoozeRepository` / `CachedFeatureAllowlistRepository` (account-keyed for the allowlist), `ButtonHealthDisplay.Loading` → `Hidden`, shared snooze-expiry derivation (also fixes the pre-existing iOS stale "Snoozing until" label), `SnoozeDoorEventBridge`, `SignOutCacheClearManager`, and two new guardrail checks (`checkBackupRulesExcludes`, hardened `checkDataStoreSingleton`). Each substantial PR passed an adversarial agent-review round; confirmed findings fixed forward in #1073/#1077/#1080. Also rides along: #1052 (cleartext localhost exceptions restricted to debug builds) and the #1033/#1054 dependency bumps. No new capability; patch.

## 2.20.3
- **More reliable "Resolved: garage door closed" notification.** If the app ever can't read the details of a resolved-on-close message, it now shows the server-provided text instead of dropping the notification. In practice this shouldn't happen; the only difference in that fallback case is the open/close times aren't localized to your device's timezone. No change to *when* notifications fire or to the live door state. This release also refreshes the Play Store "What's New" to describe the single-card open-door / resolved notification improvement that is now live.
- **Internal:** #1042 — `DoorNotificationPresenter.show(data, fallbackTitle, fallbackBody)` renders the server-sent notification title/body when the structured `DoorResolvedPayload` parse returns null (defensive; documented as should-not-happen, the only degradation being device-local time formatting). `FCMService` threads `remoteMessage.notification?.title`/`?.body` through `FcmMessageHandler` into the presenter. Pairs with the relaxed-A combined notification+data resolved message (`server/32`+, live since 2026-07-03 via `warningReplaceTagEnabled` + `resolvedNotificationPayloadEnabled`). Whatsnew refreshed in #1046. No new capability; patch. See [`docs/RESOLVED_NOTIFICATION_NO_COMPROMISE.md`](../../docs/RESOLVED_NOTIFICATION_NO_COMPROMISE.md).
...
## 2.20.0
- **Open-door warnings now appear while the app is open, and on their own "Garage door" channel.** Previously the "garage door left open" warning was only shown when the app was in the background; if the app happened to be open when the warning fired, it was silently dropped. Now it always shows. The warning (and the resolved-on-close notification) also moved off Android's generic "Miscellaneous" notification category onto a dedicated **"Garage door"** channel with the garage icon, so it shows a heads-up banner consistently and you can control it on its own in system settings. No change to *when* the server decides to warn you, to live door-state updates, or to old app versions.
- **Internal:** R6 + M4. `FCMService.onMessageReceived` now renders a foreground notification-payload warning via `DoorNotificationPresenter.showWarning(...)`; both warning and resolved share one HIGH-importance channel + (tag, id) slot (so the resolved can replace the warning in place). Manifest `default_notification_channel_id`/`default_notification_icon` route the OS-rendered background warning to the same channel + a new `ic_notification_garage` status-bar icon; the channel is created eagerly in `GarageApplication.onCreate`. Channel id is a shared string resource (`door_notification_channel_id`) referenced by both the manifest and the presenter to prevent drift. Notification display is device-only behavior; the app-built presenter reuses the path proven on-device by the 2.18.0 test-notification sandbox. Residual: a *background* warning still uses FCM's own notification tag, so the resolved replaces an *in-place* (foreground) warning but coexists with a background-rendered one until the warning itself is app-built (Phase 2). See [`docs/NOTIFICATION_RELIABILITY.md`](../../docs/NOTIFICATION_RELIABILITY.md) (R6, M4) and [`docs/RESOLVED_NOTIFICATION_PLAN.md`](../../docs/RESOLVED_NOTIFICATION_PLAN.md).

## 2.19.0
- **Resolved-on-close notification (client side, dormant until the server enables it).** When the garage door closes after an open-door warning, an updated app can show a "Resolved: garage door closed — it was open for X minutes (start-end time)" notification, built by the app on a dedicated "Garage door" channel. This is **purely additive**: the app subscribes to a new `door_open_v2-` topic alongside the existing door subscription; the live door-state updates and the existing open-door warning are completely unchanged. Nothing appears until the server-side flag is turned on (server-controlled, instant on/off). Old app versions are unaffected. Pairs with `server/31`. Design: [`docs/RESOLVED_NOTIFICATION_PLAN.md`](../../docs/RESOLVED_NOTIFICATION_PLAN.md).
- **Internal:** additive `DoorResolvedFcmSubscriptionManager` + `FirebaseDoorResolvedFcmRepository` (own-topic prefix guard, never touches the legacy door subscription), `DoorNotificationPresenter` + pure `DoorResolvedNotificationText` formatter (device-timezone local times), `DoorResolvedPayload` parser, `door_open_v2-` topic + payload pinned by `FcmTopicTest` + `wire-contracts/openDoorResolved/`. Client is flag-agnostic (renders whatever arrives) so revert is purely server-side.
...
- **Internal:** `DoorEventPage` + `PaginationState` domain types; repo-owned `paginationState` StateFlow (ADR-022) with append-vs-replace cache (no Room migration); `FetchOlderDoorEventsUseCase`; `HistoryContent` scroll-to-end trigger + footer. Design: ADR-028 + [`docs/EVENT_HISTORY_PAGINATION.md`](../../docs/EVENT_HISTORY_PAGINATION.md). PRs #869, #871, #876.

## 2.16.37
- **The door icon now plays its open/close slide on cold open.** Previously, opening the app (or first viewing the icon) while the door was OPENING or CLOSING rendered the icon already at the end position (fully open / fully closed) with only the arrow overlay — the slide never played unless you happened to be watching when the state changed. Now a motion event seen for the first time replays the full slide from the start, **regardless of how long the door has been in that state** (it is not seeded mid-motion — clock drift would put the icon at a wrong position; see ADR-025).
- **Re-entry still snaps — no repetitive replay.** Switching tabs or navigating back to a screen where the *same* motion event is still current snaps to the target instead of replaying the slide. This preserves the fix that originally motivated the 2.16.4 behavior; the difference is now "replay once per event" instead of "never replay" or "replay on every composition."
- **How it works.** A presentation-layer `DoorAnimationMemory` (`remember`ed at the Compose root, provided via `LocalDoorAnimationMemory` — no DI / domain involvement) records a `DoorMotionKey(doorPosition, lastChangeTimeSeconds)` per animated event. Live state changes during the icon's lifetime (including mid-slide direction flips) still animate from the current value, so smooth reversal is unchanged.
- **Internal:** `DoorAnimation.initialPositionFor` removed, replaced by `fromPositionFor` (the "from" end). `GarageIcon` takes a new `lastChangeTimeSeconds` param (threaded from `HomeStatusDisplay`). The 2.16.35 instrumented audit (`GarageDoorAnimationBehaviorTest`) was updated to pin the new matrix: fresh NEW event slides from the start; re-entry of an already-animated event snaps; live transitions and direction flips unchanged. ADR-025 amended; `DOOR_ANIMATION.md` updated.

## 2.16.36
- **Snooze save failures are now visible.** Audit-discovered fix: `SnoozeAction.Failed.*` states were being computed by `ProfileViewModel` and auto-reset 10s later, but **never rendered anywhere** — `ProfileContent` only read `is SnoozeAction.Sending` for the spinner. Snooze failures were silent: the user saw the spinner stop and got no confirmation of either success or failure. `ProfileContent` now hosts a `SnackbarHost` + `LaunchedEffect(snoozeAction)` that dispatches a per-variant message on each transition into `Failed.*`.
- **New typed error for the event-mismatch case.** The server returns HTTP 404 specifically when the `snoozeEventTimestamp` the client sent no longer matches the current door event — the dominant snooze failure mode during `OPENING`/`CLOSING` transitions and stuck-in-motion windows (see [`docs/SNOOZE_BEHAVIOR.md`](../../docs/SNOOZE_BEHAVIOR.md)). Previously this collapsed into a generic `ActionError.NetworkFailed`. Now: new `ActionError.SnoozeEventChanged` + `SnoozeAction.Failed.EventChanged` variants; `NetworkSnoozeRepository.doSnoozeNotifications` branches on HTTP 404 → `SnoozeEventChanged`; other HTTP errors keep the generic `NetworkFailed` mapping. The user sees "Door state changed before snooze could apply. Try again." instead of a vague generic error.
- **Other Failed.* variants also get specific copy** in the same Snackbar dispatch: "Snooze did not apply. Check your connection and try again." (NetworkError), "Sign in to snooze notifications." (NotAuthenticated), "No recent door event available. Try again in a moment." (MissingData).
- **Internal:** new `ActionError.SnoozeEventChanged` requires a defensive `when` arm in `HomeViewModel` (the push flow never produces this snooze-specific variant, but the shared sealed type needs exhaustive coverage). Comment notes the case is unreachable in practice. 3 new unit tests (`NetworkSnoozeRepositoryTest.submit404MapsToSnoozeEventChanged`, `submit500MapsToNetworkFailedNotSnoozeEventChanged`, `ProfileViewModelTest.snoozeFailureSnoozeEventChangedMapsToFailedEventChanged`). Server design + this mapping captured in [`docs/SNOOZE_BEHAVIOR.md`](../../docs/SNOOZE_BEHAVIOR.md) (introduced in PR #850, refined in PR #851).
