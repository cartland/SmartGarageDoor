...
import com.chriscartland.garage.presentation.HistoryMapper.MergedRecord
...
/**
 * Moved from `androidApp/.../ui/history/HistoryMapperTest.kt` to commonTest
 * (ADR-031) so the merge / dedup / duration / grouping rules are verified on
 * every platform. java.time was swapped for raw epoch seconds + kotlinx-datetime
 * (`timeZoneId = "UTC"`); the assertions are otherwise identical.
 */
class HistoryMapperTest {
    private val timeZoneId = "UTC"
    private val nowEpochSeconds = Instant.parse("2026-04-29T17:30:00Z").epochSeconds
    ...
    fun dedup_emptyInput() {
        assertEquals(emptyList(), HistoryMapper.dedupConsecutive(emptyList()))
    }
    ...
    fun merge_emptyInput() {
        assertEquals(emptyList(), HistoryMapper.mergeEvents(emptyList()))
    }
    ...
    fun durations_emptyInput() {
        assertEquals(emptyList(), HistoryMapper.computeDurations(emptyList(), nowEpochSeconds))
    }

    @Test
    fun durations_singleOpened_isCurrent_durationToNow() {
        val rec = MergedRecord.Opened(timeSeconds = nowEpochSeconds - 600, transitDurationSeconds = null)
        val out = HistoryMapper.computeDurations(listOf(rec), nowEpochSeconds)
    ...
    fun durations_singleClosed_isCurrent_durationToNow() {
        val rec = MergedRecord.Closed(timeSeconds = nowEpochSeconds - 1800, transitDurationSeconds = null)
        val out = HistoryMapper.computeDurations(listOf(rec), nowEpochSeconds)
        assertTrue(out[0].isCurrent)
    ...
    fun durations_openedThenClosed_openedHasFiniteDuration_closedIsCurrent() {
        val opened = MergedRecord.Opened(timeSeconds = nowEpochSeconds - 1200, transitDurationSeconds = null)
        val closed = MergedRecord.Closed(timeSeconds = nowEpochSeconds - 600, transitDurationSeconds = null)
        val out = HistoryMapper.computeDurations(listOf(opened, closed), nowEpochSeconds)
    ...
    fun durations_openedClosedOpened_threeWithLatestOpenedCurrent() {
        val o1 = MergedRecord.Opened(timeSeconds = nowEpochSeconds - 3000, transitDurationSeconds = null)
        val c1 = MergedRecord.Closed(timeSeconds = nowEpochSeconds - 2400, transitDurationSeconds = null)
        val o2 = MergedRecord.Opened(timeSeconds = nowEpochSeconds - 600, transitDurationSeconds = null)
        val out = HistoryMapper.computeDurations(listOf(o1, c1, o2), nowEpochSeconds)
    ...
    fun durations_anomaliesPassThroughWithoutDuration() {
        val anomaly = MergedRecord.Anomaly(
            timeSeconds = nowEpochSeconds - 100,
            position = DoorPosition.UNKNOWN,
            kind = AnomalyKind.UnknownState,
        )
        val out = HistoryMapper.computeDurations(listOf(anomaly), nowEpochSeconds)
        assertEquals(1, out.size)
    ...
    fun durations_consecutiveOpenedsBoundByEachOther() {
        ...
        val o1 = MergedRecord.Opened(timeSeconds = nowEpochSeconds - 3000, transitDurationSeconds = null, misaligned = true)
        val o2 = MergedRecord.Opened(timeSeconds = nowEpochSeconds - 1800, transitDurationSeconds = null)
        val c = MergedRecord.Closed(timeSeconds = nowEpochSeconds - 600, transitDurationSeconds = null)
        val out = HistoryMapper.computeDurations(listOf(o1, o2, c), nowEpochSeconds)
        // o1 ends at o2 → 1200s
    ...
    fun durations_consecutiveClosedsBoundByEachOther() {
        ...
        val c1 = MergedRecord.Closed(timeSeconds = nowEpochSeconds - 3000, transitDurationSeconds = null)
        val c2 = MergedRecord.Closed(timeSeconds = nowEpochSeconds - 1800, transitDurationSeconds = null)
        val o = MergedRecord.Opened(timeSeconds = nowEpochSeconds - 600, transitDurationSeconds = null)
        val out = HistoryMapper.computeDurations(listOf(c1, c2, o), nowEpochSeconds)
        assertEquals(1200L, out[0].durationSeconds)
        assertEquals(1200L, out[1].durationSeconds)
        assertTrue(out[2].isCurrent)
    }
    ...
    fun durations_anomalyBetweenTerminalsDoesNotInterfere() {
        val o = MergedRecord.Opened(timeSeconds = nowEpochSeconds - 3000, transitDurationSeconds = null)
        val a = MergedRecord.Anomaly(timeSeconds = nowEpochSeconds - 1500, position = DoorPosition.UNKNOWN, kind = AnomalyKind.UnknownState)
        val c = MergedRecord.Closed(timeSeconds = nowEpochSeconds - 600, transitDurationSeconds = null)
        val out = HistoryMapper.computeDurations(listOf(o, a, c), nowEpochSeconds)
    ...
    fun dayLabel_yesterday() {
        val today = LocalDate(2026, 4, 29)
        val yesterday = LocalDate(2026, 4, 28)
    ...
    fun dayLabel_twoDaysAgo() {
        val today = LocalDate(2026, 4, 29)
        val twoAgo = LocalDate(2026, 4, 27)
        assertEquals(DayLabel.Date(2026, 4, 27), HistoryMapper.dayLabel(twoAgo, today))
    }

    @Test
    fun dayLabel_lastWeek() {
        val today = LocalDate(2026, 4, 29)
        val lastWeek = LocalDate(2026, 4, 22)
        assertEquals(DayLabel.Date(2026, 4, 22), HistoryMapper.dayLabel(lastWeek, today))
    }
    ...
    @Test
    fun e2e_emptyInput() {
        assertEquals(emptyList(), HistoryMapper.toHistoryDays(emptyList(), nowEpochSeconds, timeZoneId))
    }
    ...
    fun e2e_eventsWithNullPositionFiltered() {
        val events = listOf(
            ...
            event(DoorPosition.OPEN, nowEpochSeconds - 600),
        )
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_singleOpenToday_currentlyOpenRow() {
        val events = listOf(event(DoorPosition.OPEN, nowEpochSeconds - 720))
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
        assertEquals(1, days.size)
        assertEquals(DayLabel.Today, days[0].label)
    ...
    fun e2e_singleClosedToday_currentlyClosedRow() {
        val events = listOf(event(DoorPosition.CLOSED, nowEpochSeconds - 720))
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_pastOpenedRow_durationDescribesOpenSpan() {
        val openedTime = nowEpochSeconds - 1800 // 30 min ago
        val closedTime = nowEpochSeconds - 1080 // 18 min ago — open for 12 min
        val events = listOf(
    ...
    fun e2e_warningTagForOpeningTooLong() {
        val events = listOf(
            event(DoorPosition.OPENING_TOO_LONG, nowEpochSeconds - 1500),
            event(DoorPosition.OPEN, nowEpochSeconds - 1260), // 4 min later
            event(DoorPosition.CLOSED, nowEpochSeconds - 600),
        )
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_warningTagForClosingTooLong() {
        val events = listOf(
            event(DoorPosition.OPEN, nowEpochSeconds - 1800),
            event(DoorPosition.CLOSING_TOO_LONG, nowEpochSeconds - 800),
            event(DoorPosition.CLOSED, nowEpochSeconds - 620), // 3 min later
        )
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_misalignedMergesIntoPreviousOpenedRow() {
        val events = listOf(
            event(DoorPosition.OPEN, nowEpochSeconds - 3000),
            event(DoorPosition.OPEN_MISALIGNED, nowEpochSeconds - 2400),
            event(DoorPosition.OPEN, nowEpochSeconds - 1800),
            event(DoorPosition.CLOSED, nowEpochSeconds - 600),
        )
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_misalignedAfterClosedFallsBackToAnomalyRow() {
        ...
        val events = listOf(
            event(DoorPosition.OPEN, nowEpochSeconds - 3000),
            event(DoorPosition.CLOSED, nowEpochSeconds - 1800),
            event(DoorPosition.OPEN_MISALIGNED, nowEpochSeconds - 600),
        )
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_consecutiveOpenedsDoNotOverlapDurations() {
        ...
        val t1 = nowEpochSeconds - 3000
        val t2 = nowEpochSeconds - 1800
        val tClosed = nowEpochSeconds - 600
        val events = listOf(
    ...
    fun e2e_currentlyOpenAndMisaligned_headlineSaysOpenMisaligned() {
        val events = listOf(
            event(DoorPosition.OPEN, nowEpochSeconds - 600),
            event(DoorPosition.OPEN_MISALIGNED, nowEpochSeconds - 300),
        )
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_unknownStateAnomaly() {
        val events = listOf(event(DoorPosition.UNKNOWN, nowEpochSeconds - 600))
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_sensorConflictAnomaly() {
        val events = listOf(event(DoorPosition.ERROR_SENSOR_CONFLICT, nowEpochSeconds - 600))
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
        val anomaly = days[0].entries.single() as HistoryEntry.Anomaly
    ...
    fun e2e_stuckOpeningAnomalyWhenNeverResolved() {
        val events = listOf(event(DoorPosition.OPENING_TOO_LONG, nowEpochSeconds - 600))
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
        val anomaly = days[0].entries.single() as HistoryEntry.Anomaly
    ...
    fun e2e_stuckClosingAnomalyWhenNeverResolved() {
        val events = listOf(event(DoorPosition.CLOSING_TOO_LONG, nowEpochSeconds - 600))
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
        val anomaly = days[0].entries.single() as HistoryEntry.Anomaly
    ...
    fun e2e_eventsSpanMultipleDays_groupedNewestFirst() {
        val today = nowEpochSeconds - 600 // 17:20 UTC today
        val yesterday = Instant.parse("2026-04-28T20:30:00Z").epochSeconds
        val twoAgo = Instant.parse("2026-04-27T07:15:00Z").epochSeconds
        val events = listOf(
        ...
        assertEquals(DayLabel.Date(2026, 4, 27), days[2].label)
    }
    ...
    fun e2e_dedupCollapsesConsecutiveDuplicates() {
        ...
        val events = listOf(
            event(DoorPosition.OPEN, nowEpochSeconds - 1800),
            event(DoorPosition.OPEN, nowEpochSeconds - 1500),
            event(DoorPosition.OPEN, nowEpochSeconds - 1200),
            event(DoorPosition.CLOSED, nowEpochSeconds - 600),
        )
        val days = HistoryMapper.toHistoryDays(events, nowEpochSeconds, timeZoneId)
    ...
    fun e2e_inputOrderIrrelevant_outputDeterministic() {
        ...
        val a = event(DoorPosition.OPEN, nowEpochSeconds - 1800)
        val b = event(DoorPosition.CLOSED, nowEpochSeconds - 600)
        val days1 = HistoryMapper.toHistoryDays(listOf(a, b), nowEpochSeconds, timeZoneId)
        val days2 = HistoryMapper.toHistoryDays(listOf(b, a), nowEpochSeconds, timeZoneId)
    ...
    fun e2e_currentlyClosedAfterMisaligned_misalignedAppearsAfterClosed() {
        ...
        val yOpen = Instant.parse("2026-04-28T18:30:00Z").epochSeconds
        val yMisaligned = Instant.parse("2026-04-28T22:30:00Z").epochSeconds
        val tClosed = Instant.parse("2026-04-29T16:00:00Z").epochSeconds
        val events = listOf(
