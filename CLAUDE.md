...
**Bootstrap state (as of 2026-07-22, wear/1):** `release-wear.yml` always uploads the signed AAB as a **1-day CI artifact**; the Play upload step (`tracks: wear:internal`) runs only when the repo Actions variable `WEAR_PLAY_UPLOAD_ENABLED` is `'true'`. The variable stays unset until the one-time manual Play Console bootstrap completes (Wear OS form-factor opt-in + first manual AAB upload), and the `wear:internal` track identifier is verified via the `play-track-snapshot` log (the snapshot resolver maps wear versionCodes back to `wear/N` tags). An expired release artifact is regenerated by re-running the same tag's workflow run — no new tag needed. Wear release builds have **R8 minification deliberately OFF** (ADR-020 keep rules not yet ported/verified for Wear) — fine for internal testing, revisit before wider rollout.
