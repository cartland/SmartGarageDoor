...
- **A live config flag's value** (e.g. `resolvedOnCloseEnabled`) is in Firestore `configCurrent/current` — it is **not** knowable from the repo. There is now an authoritative read path for the editable boolean flags: **`scripts/set-config-flag.sh --list`** (needs a Firebase ID token in `GARAGE_ID_TOKEN`, from the app's "Copy auth token"; deployed `server/33`+). It returns each editable flag's current live value — use it instead of asking/recalling. Verified live 2026-07-03: `resolvedOnCloseEnabled=true`, `snoozeNotificationsEnabled=true`, `remoteButtonEnabled=true`, and the single-card flags `warningReplaceTagEnabled`/`resolvedNotificationPayloadEnabled` both `false` (dark). Never infer a flag from a doc that says "off by default."
...
- **Single-card "relaxed-A" groundwork — SHIPPED DARK in `server/33`, is the go-forward instead of Phase 2.** Rather than Phase 2's risky "move the warning to app-built data-only" (which drags the primary state-sync onto a new topic → silent-dark risk), the relaxed-A design gets the never-woken single card by keeping the warning OS-rendered and just (a) tagging it and (b) making the resolved a combined notification+data message on `door_open_v2-` sharing that tag, so the OS collapses them. Two live flags, both **default off** (verify via `set-config-flag.sh --list`): **`warningReplaceTagEnabled`** (tags the `door_open-` warning; the ONLY old-app-observable change) + **`resolvedNotificationPayloadEnabled`** (combined resolved). A multi-agent impossibility proof (strict zero-old-app-change makes the never-woken single card impossible) + the relaxed-A design + the adversarial verification (no old-app degradation) live in [`docs/RESOLVED_NOTIFICATION_NO_COMPROMISE.md`](docs/RESOLVED_NOTIFICATION_NO_COMPROMISE.md). **Device gate: PASS on Pixel (2026-07-02)** via the sandbox `--mode notification|combined` — a same-tag warning replace re-alerts (→ no old-app degradation), and warning→combined-resolved collapses to one card. To enable: flip both flags true (after re-running the re-alert gate on a Samsung/Xiaomi if the fleet includes them). Enabling `resolvedNotificationPayloadEnabled` has a known accepted tradeoff — see next bullet.
