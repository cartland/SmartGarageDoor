...
- **The App Store Connect API key MUST be Admin role, not App Manager.** Cloud signing (`-allowProvisioningUpdates`) creates the Distribution cert on demand and only Admin can — an App Manager key archives fine but fails export with `Cloud signing permission error` / `No signing certificate "iOS Distribution" found`. Empirical: `ios/1` attempt 1.
