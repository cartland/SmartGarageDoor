...
- **Flow vs StateFlow in UseCase return types — match the upstream, never wrap.** Default to `Flow<T>` so lifecycle and coroutine-scope ownership are explicit. Return `StateFlow<T>` only when the UseCase is a direct pass-through of an upstream `StateFlow` (no `.map`, no transformation) — and even then, the cache semantics are the repository's, not the UseCase's. **Never `stateIn(...)` a UseCase Flow inside a ViewModel that itself exposes a `StateFlow` derived from the same upstream** — the wrap-then-rewrap layer hides timing bugs (PR #283 race). Pass the reference through. Canonical pattern in this repo: `ObserveAuthStateUseCase` and `ObserveSnoozeStateUseCase` return `StateFlow` because the repo's flow IS a StateFlow and the VM exposes it by reference (ADR-022); `ObserveDoorEventsUseCase.current()` returns `Flow` because the repo exposes `Flow`; `ObserveFeatureAccessUseCase.functionList()` returns `Flow` because it does `.map { ... }` (which can't preserve `StateFlow`)
...
- **Adaptive layout decisions go through `AppLayoutMode`, never raw `LocalAppWindowSizeClass.current`.** `AppLayoutMode` (in `androidApp/.../ui/AppLayoutMode.kt`) is a sealed type — `Compact` and `Wide` — that owns three properties consumers query: `visibleTabs` (which tabs the bottom nav shows in this mode), `mergedRoutes` (back-stack-entry redirects, e.g. `History → Home` on Wide), and the activation threshold (one condition in `Companion.fromWidthSizeClass`, today `WindowWidthSizeClass.Medium`+ for Wide). Composables call `currentAppLayoutMode()` or take `AppLayoutMode` as a parameter and pattern-match on it. Adding a screen, changing the threshold, or introducing a third mode (e.g. an Expanded-only three-pane treatment) is a single-file edit — the sealed-type `when` exhaustiveness forces every consumer to handle the new case. Enforced by `checkAppLayoutModeBoundary` (in `validate.sh`): direct reads of `LocalAppWindowSizeClass.current` are forbidden outside `AppLayoutMode.kt` itself. The dispatch table is `RouteEntryFor` in `Main.kt` — a `when (canonicalScreen)` arm per `Screen.X` that picks the right wrapper (`RouteContent` vs `DashboardRouteContent`) and body. Five `entry<Screen.X>` blocks are uniform single-line `entry<X> { routeFor(X) }` calls. Pre-2.15.0's first iteration scattered four `LocalAppWindowSizeClass.current.widthSizeClass >= Medium` reads across `Main.kt`; the consolidation landed in `android/210` (#704).
