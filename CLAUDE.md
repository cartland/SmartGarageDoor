...
**The watch's voice surface is a SIMULATION and must stay one (0.3.0+).** `WearVoiceViewModel` drives the real shared `VoiceCommandController` / classifier / gate against `SimulatedVoiceCommandEnvironment`, so it names the action it would take and then reports that nothing was sent. Do **not** wire it to `PushRemoteButtonUseCase` — promoting the watch to the real door is a deliberate product decision, not a refactor. Three tests enforce it (`WearVoiceViewModelTest.cannotReachTheRealRemoteButton` by reflection over the constructor, `WearComponentGraphTest.theOnlyVoiceEnvironmentIsSimulated`, and `:usecase`'s `SimulatedVoiceCommandEnvironmentTest`), plus two compile-time barriers (the kotlin-inject provider and the test both construct the VM explicitly). Holding the door on the hero screen remains the only path to the real garage button. Design + rejected trigger alternatives: [`docs/WEAR_OS.md`](docs/WEAR_OS.md) § Voice demo.
