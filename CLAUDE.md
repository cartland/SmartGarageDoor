...
- **Android App** (MobileGarage/): Kotlin/Compose mobile app with MVVM architecture

Two more clients share MobileGarage's KMP modules: the **iOS app** (MobileGarage/iosApp/, SwiftUI over `:iosFramework`) and the **Wear OS app** (MobileGarage/wearApp/, Compose for Wear M3 — a single hero screen; see [`docs/WEAR_OS.md`](docs/WEAR_OS.md)).

**Key principle**: Server handles all critical business logic. Clients (hardware and mobile) are kept simple.

## Build Commands

### Android App (MobileGarage/)
> The Gradle wrapper lives at `MobileGarage/gradlew`, and `cd` is hook-blocked. From the repo root (where you run everything) invoke Gradle as **`MobileGarage/gradlew -p MobileGarage <task>`** — there is no root `./gradlew` (the `./gradlew` shown below assumes the `MobileGarage/` working dir). Most work goes through `scripts/validate.sh` / `scripts/generate-ios-screenshots.sh`, which wrap this for you.
```bash
# Debug builds
...
- **DI**: kotlin-inject (`AppComponent`) — Hilt fully removed. See `MobileGarage/docs/archive/DI-MIGRATION.md`
...
- When every changed file matches `**/*.md`, `docs/**`, `.claude/**`, `LICENSE`, `.gitignore`, or `MobileGarage/distribution/whatsnew/**`, the `checks` reusable workflow is skipped and the gate jobs post success in ~20–30s.
...
```bash
git checkout --ours MobileGarage/android-screenshot-tests/SCREENSHOT_GALLERY.md
...
When modifying Room entities or DAOs: (1) increment the `version` in `@Database` (`AppDatabase.kt`); (2) run `./gradlew :androidApp:assembleDebug` to regenerate the schema; (3) commit the new schema JSON alongside the code change; (4) **declare a `Migration` or `@AutoMigration(from = N, to = N+1)` to preserve data.** Full safeguard list (schema drift check in `validate.sh`, guardrails hook, `RoomSchemaTest` contract tests, exported schema files): see `MobileGarage/docs/ARCHITECTURE.md` § Database.
...
When modifying `AppComponent.kt`: run `./gradlew -p MobileGarage checkSingletonCaching` (in `validate.sh`); inspect `androidApp/build/generated/ksp/debug/.../InjectAppComponent.kt` if it fails. New `@Singleton` state-owning providers need a matching `abstract val` + an `assertSame` identity test in `ComponentGraphTest`.

**Adding a provider can tip detekt `LargeClass` on `AppComponent`.** `AppComponent` carries `@Suppress("LargeClass")` (justified in its class KDoc — a DI graph is a flat `@Provides` list, not a complex god-class). If you removed it, adding the next provider would fail `:androidApp:detekt`. Keep the suppression; don't "fix" the size by splitting the component (that fights kotlin-inject's single-component model). Caught in PR #969 (a `BuildAppLogCsvUseCase` provider tipped it). Two related formatting traps hit in the same PR, both auto-fixed by `./gradlew -p MobileGarage spotlessApply` before push: a multi-line provider edit needs reformatting (the broader form of the iosFramework-spotless trap — run `spotlessApply` after any hand-written multi-line Kotlin edit), and an EOL `//` comment may not directly follow a KDoc (ktlint `no-consecutive-comments` — put the justification *inside* the KDoc and leave only the bare annotation).

Full validation procedure, the android/170 postmortem that motivated these rules, and the failure-mode catalog: see `MobileGarage/docs/DI_SINGLETON_REQUIREMENTS.md` and the kotlin-inject pattern guide at `MobileGarage/docs/guides/kotlin-inject.md`.

**Three DI components mirror each other by hand — and `validate.sh` fully checks only one.** A class in a shared KMP module (`viewmodel/`, `usecase/`, `data/`) can have a provider in **any of** `androidApp/.../di/AppComponent.kt` (Android, `@Singleton`), `iosFramework/.../NativeComponent.kt` (iOS, `@SharedSingleton`), **and** `wearApp/.../di/WearComponent.kt` (Wear, `@WearSingleton` — it wires shared repos like `FirebaseAuthRepository`, `NetworkDoorRepository`, `NetworkRemoteButtonRepository`). When you change such a class's constructor (add/remove a param), you MUST update **every component that wires it** in the same PR — grep all three for the matching `provide*`. The trap: `./scripts/validate.sh` compiles Android targets (which includes `wearApp`'s Kotlin) but **not** the iosFramework iOS targets, so a stale `NativeComponent` provider passes validate.sh *and* the Android required checks clean. Only the separate **iOS CI** (`Build iOS app + framework test`) catches it, and iOS CI is **not** a required status check — so a PR can auto-merge with a broken iOS `main`. Empirical: PR #871 (door-history load-more) updated only `AppComponent` → broke iOS on main (`No value passed for parameter 'fetchOlderDoorEventsUseCase'`) → hotfix #873.
...
Full rule, code example, and ADR-022 compatibility argument: see `MobileGarage/docs/DECISIONS.md` ADR-023.
...
Enforced by `./gradlew -p MobileGarage checkScreenViewModelCardinality`, hooked into `validate.sh`. **`MobileGarage/screen-viewmodel-exemptions.txt` is empty as of `android/213` (2.15.3)** — every screen complies with the 1:1 rule. The check still fails when an exempt entry has been refactored to comply but stays in the file (so stale entries don't accumulate); keep the file empty unless a real, intentional, time-boxed exemption is added.
...
**The complement (ADR-033, hard rule + enforced): UI-triggered work goes through the ViewModel — UI code must NOT reach a UseCase/Repository directly off the DI `component`.** Together these are total: UI-triggered → ViewModel; screen-less → UseCase-direct; nothing in between. A UseCase is exposed as a component entry point (`abstract val xUseCase`) ONLY for (a) a UI-less caller (FCM/`AppStartup`) or (b) `@Singleton`-DI machinery (kotlin-inject needs the `abstract val` to honor the scope — e.g. `computeButtonHealthDisplayUseCase`); anything else there is a smell. Platform side-effects that can't live in the shared KMP VM (clipboard write, file write, file-picker, share sheet) stay in the UI: the **VM owns the produce/decision** (fetch the token, build the CSV) and hands the value back; the **UI does only the irreducible platform write** — so the UI's *entry point* is a VM method, not a UseCase. Enforced by `checkUiLayerNoGraphAccess` (`buildSrc/.../architecture/UiLayerNoGraphAccessTask.kt`, in `validate.sh`): scans `androidApp/.../ui/` for the member-access pattern `component.<x>UseCase`/`component.<x>Repository` (member-access, NOT imports — a `:usecase` *data class* like `ButtonHealthDisplay` is legitimately imported by UI). `MobileGarage/ui-layer-graph-access-exemptions.txt` grandfathers known pre-rule violations during burn-down (fails on a stale entry; goal = empty). iOS (unlinted by buildSrc) follows the same rule by convention + review. **Burn-down in progress:** copy-auth-token + Export CSV are being routed through their VMs; until then their two files are exempted.

When adding a new screen: write the ViewModel first (in `usecase/.../<X>ViewModel.kt`), wire it into `AppComponent.kt` as a non-`@Singleton` `@Provides`, then resolve it from the screen via `viewModel { component.<x>ViewModel }`. Full rationale in `MobileGarage/docs/DECISIONS.md` ADR-026.

### Konsist — redundant architecture enforcement (additive to buildSrc)

[Konsist 0.17.3](https://github.com/LemonAppDev/konsist) is a `testImplementation` dep of `:androidApp` (added 2026-05-22 via PRs #836 + #837). The Konsist tests live under `MobileGarage/androidApp/src/test/java/com/chriscartland/garage/konsist/`.
...
**Konsist tests don't re-trigger on bytecode-identical source edits (empirical, 2026-08-15).** Konsist reads SOURCES at runtime, but Gradle's up-to-date check for `:androidApp:testDebugUnitTest` tracks the compiled classpath. A source edit that produces identical bytecode — deleting an explicit type argument that inference reproduces, editing comments — does not rerun the Konsist tests locally, so a probe of a rule can silently report stale green (hit while teeth-checking `ViewModelDomainMirrorKonsistTest`: the stripped annotation "passed" until `--rerun-tasks` forced a real run, which failed correctly). **When verifying that a Konsist rule fires, always run with `--rerun-tasks`.** Real violations add declarations, which change bytecode and re-trigger normally; CI always runs fresh, so this is a local-probe hazard, not a CI gap.
...
- **versionCode ↔ tag:** each `android/N` tag has `versionCode == N`, so the snapshot maps a code back to its `versionName` via `git show android/N:MobileGarage/version.properties` (needs `fetch-depth: 0`).
...
- **A live config flag's value** (e.g. `resolvedOnCloseEnabled`) is in Firestore `configCurrent/current` — it is **not** knowable from the repo. There is now an authoritative read path for the editable boolean flags: **`scripts/set-config-flag.sh --list`** (needs a Firebase ID token in `GARAGE_ID_TOKEN`, from the app's "Copy auth token"; deployed `server/33`+). It returns each editable flag's current live value — use it instead of asking/recalling. Live snapshot 2026-07-03 (re-check with `--list`; values drift): `resolvedOnCloseEnabled=true`, `warningReplaceTagEnabled=true`, `resolvedNotificationPayloadEnabled=true`, `snoozeNotificationsEnabled=true`, `remoteButtonEnabled=true` — i.e. **all five on; the relaxed-A single-card notification is now ENABLED + LIVE in production** (the two single-card flags were flipped `true` 2026-07-03; no Android release needed — 2.20.2 already handles it; revert = flip either `false`, instant). Never infer a flag from a doc that says "off by default."
...
The release workflows only ship the AAB + `whatsnew/`, never images. **Screenshots** are uploaded by the dispatch-only **`Play Listing Images`** workflow (`gh workflow run play-listing-images.yml -f image_types=wearScreenshots -f apply=false` to rehearse, then `apply=true`); the **icon and feature graphic are still a manual Console step**. The procedure is the **`play-store-assets` skill** (`/play-store-assets`); read it before touching store assets. Shipped 2026-06-11 in PRs #882 (icon + feature graphic), #883 (generators + skill + staging), #884 (screenshots promoted to distribution); screenshot upload automated 2026-07-27 (#1138).

**`apply=false` is a real rehearsal, not a simulation.** Play edits are transactional, so a dry run performs the whole `deleteall` + upload into an edit and then abandons it — proving auth, the "Edit store listing" permission, image dimensions and count limits with zero public effect. Always dry-run first. Two guards live in `.github/scripts/lib/listing-images.mjs` (unit-tested, and the tests are part of the required `Script unit tests` check): an **empty set is refused** rather than treated as a no-op (syncing is `deleteall` + upload, so a typo'd path matching zero files would otherwise silently wipe that section of the live listing), and **only the curated tree is eligible** (the generated staging tree also has a `wear/` directory, so directory-name matching alone would publish un-curated candidate shots).

**Two-location model:**
- `MobileGarage/screenshots/store/` — **generated** staging (all candidates), committed, with an auto-generated `README.md` gallery. Regenerated by the generators; never hand-edited.
- `MobileGarage/distribution/playstore/` — **curated** set that mirrors what's live in the store (committed). Updated by hand: copy from staging → PR → manual upload. **No generator writes into `distribution/`.**

**Generators:**
- `MobileGarage/distribution/playstore/generate.sh` — icon-512 + feature graphic → staging (from `src/icon.svg`, a faithful port of `GarageDoorCanvas.kt`); also regenerates the **in-app launcher mipmaps** in `androidApp/.../res/` (app code, shipped in the AAB — distinct from the store-listing icon).
...
**Flipping a boolean feature flag — use the SAFE tool, not raw Firestore (`server/33`+).** `scripts/set-config-flag.sh <flag> <true|false>` (and `--list` to read) hits the `configFlags` endpoint, which **structurally cannot clobber** `buildTimestamp`/secrets/allowlists: it writes only one key from a hardcoded allowlist (`resolvedOnCloseEnabled`, `warningReplaceTagEnabled`, `resolvedNotificationPayloadEnabled`, `snoozeNotificationsEnabled`, `remoteButtonEnabled`), as a read-modify-write that preserves every other field, and refuses to write if `buildTimestamp` is absent. **Auth = a Firebase ID token** (`GARAGE_ID_TOKEN`, from the app's "Copy auth token", Android or iOS) whose verified email is in `body.configFlagAdminAllowedEmails` (console-edited, deny-all when unset — the one thing the tool can't set, by design). This is the preferred way to toggle any of those flags; `httpServerConfigUpdate` (whole-doc overwrite) stays reserved for the non-flag fields (allowlists, timestamps, secrets) via the console or a full-body POST. Design + the ID-token-vs-shared-key rationale: [`docs/FIREBASE_CONFIG_AUTHORITY.md`](docs/FIREBASE_CONFIG_AUTHORITY.md) § "Toggling a boolean feature flag"; endpoint at `FirebaseServer/src/functions/http/SetConfigFlag.ts`.

**`doorCommand` endpoint — FOR VOICE, verdict-only, and structurally unable to press the button.** `httpDoorCommand` (`FirebaseServer/src/functions/http/DoorCommand.ts`) takes `{"command":"open"|"close"}` and answers whether that command is actionable against the door's real state, moving a rule that currently lives on each client onto the server (CLAUDE.md § Server-Centric Design). It runs **in parallel to `addRemoteButtonCommand`** and does not replace it. **Voice is the intended and only caller: a spoken sentence names a direction, which is the thing this gate judges. Screen taps do not and must NOT be routed here** — the remote button is a toggle (one press, no direction), so the phone's tap-to-confirm and the watch's press-and-hold mean "act on the door" and keep using `addRemoteButtonCommand` unchanged. Pointing these verdicts at a tap would refuse valid presses: with the door open a tap is fine (it closes), while `OPEN` as a *command* is correctly refused as already-open. Tapping remains the primary way to work the door. **It never touches the door**: the device opens the garage by polling `RemoteButtonCommandDatabase`, and this module never imports that database, so `executed` is the literal `false` with no branch that sets it otherwise. `HttpDoorCommandTest`'s *"has no route to the door"* asserts the import is still absent (verified to fail by temporarily re-adding the import — do not delete it when arming execution, re-point it). Auth is **already the real auth**, mirrored from the push-button handler in the same order (config-enabled → push-key → id-token → allowlist), deliberately so that the change which arms this endpoint touches only the acting code and not the authentication. The decision table is pure and separate (`controller/DoorCommandGate.ts`) and is pinned by the **shared** fixture `wire-contracts/doorCommand/verdict_table.json` — the Kotlin gate (`VoiceDoorStateMapper` + `VoiceCommandController.gateReason`) implements the same rule, and pointing its tests at the same file is the intended next step. The server additionally judges **check-in staleness** (11 min, mirroring `CheckInStatusMapper.STALE_THRESHOLD_SECONDS`), which is the gap the watch cannot close for itself. A refused command is a **200**, not a 4xx — 4xx is reserved for "you may not ask". Live since `server/35`; **fixed in `server/36`**, which is where it started actually working (see next bullet).

**Reading the current door state: use `SensorEventDatabase.DATABASE.getCurrentEvent()`, never `getCurrent()`.** The stored document is a *wrapper*, not a reading — `eventsCurrent/<buildTimestamp>` is `{ buildTimestamp, previousEvent, currentEvent: { type, checkInTimestampSeconds, … } }` — and `getCurrent()` hands the whole wrapper back as `any`, so `doc.type` compiles, returns `undefined`, and is indistinguishable from a door that has never reported. `model/SensorEventDocument.ts` owns the `currentEvent` key and the `readCurrentEvent(doc)` unwrap; `getCurrentEvent()` is the sanctioned read. Empirical: `httpDoorCommand` read the wrong level and refused **every** spoken command with `DOOR_STATE_UNKNOWN` for the whole of `server/35` (fixed in `server/36`).

**`getCurrentEvent` is deliberately NOT a member of the `SensorEventDatabase` interface** — it is derived from `getCurrent`, so a fake installed via `setImpl` supplies raw storage only and **cannot** implement the unwrap differently from production. That is the general shape worth copying: **when a fake can answer a document shape production never stores, a wrong fixture and a wrong reader can agree with each other and the suite goes green while the feature is dead.** Put the interpretation somewhere the fake cannot override, and it fails in tests exactly as it fails in production. The door-command fixtures had been seeded *bare* (matching the wrong reader) while `OldDataFCMFakeTest` seeded the same fake correctly all along — the fake was never the problem, the hand-written fixture was. Companion rule: **pin producer↔consumer with a round trip** (`test/model/SensorEventDocumentTest.ts` asserts what `EventUpdates` actually writes is what `getCurrentEvent` reads, stating the shape nowhere by hand). This is the storage-side analogue of `wire-contracts/`, which only pins HTTP shapes.

**The voice clients call it as a THIRD gate (Android 2.23.9 / Wear 0.7.1).** Order in `VoiceCommandController.commit()`: local gate → local re-check → **server verdict** → press. It is strictly additive — `VoiceCommandEnvironment.confirmWithServer` returns a refusal-or-null and has no return value that turns a locally-refused command into a permitted one, so consulting the server can only ever refuse, never permit. The press still goes through `addRemoteButtonCommand` unchanged; the endpoint is not on the acting path. **An unreachable server REFUSES** (`SERVER_UNREACHABLE`), which costs little because the press targets the same backend a moment later. A locally-refused command short-circuits and never makes the round trip. Two deliberate calls: a **signed-out** user PROCEEDS past this gate (being signed out is not a fact about the door; the press path's own auth gate refuses without a network call and words it as a failed send — pinned by `HomeViewModelTest.voiceCommandCommitWhileSignedOutFailsWithoutPressing`), and the check runs while still `Armed` so it stays cancellable rather than inventing a fourth visible state. **The watch is the real winner**: the server judges check-in staleness, which the watch has never been able to judge for itself — so a watch with a stale check-in now refuses commands that previously went through. The simulated voice surface implements `confirmWithServer` **locally and never touches the network**, preserving its no-route-to-the-garage property. Server rejection values map 1:1 onto `VoiceCommandIgnoreReason`, so refusals reuse existing wording; only `SERVER_UNREACHABLE` needed new copy.
...
- **A test whose assertions are all "these two things are equal" needs a positive control.** Parity/consistency suites fail open: if the function under test degenerated to returning one value for everything, every equality would pass and the suite would go green while the app was broken. Pair them with one assertion that two things must **differ**. Canonical: `WearConfirmParityTest` asserts every voice haptic matches its hold counterpart, *and* that Committed and Refused do NOT feel the same — otherwise a `constantFor` returning a single constant would satisfy the whole file. Same shape for grep- and reflection-based checks (`AppBuildFactTest.theVocabularyCheckCanActuallyFail` proves its own substring list can fire). This is the assertion-level version of the vacuous-pass family already documented for Konsist `file.name` and POSIX-ERE `\b`: **before trusting a green check, ask what it would do if the thing it inspects returned nothing at all.**
...
- **Launch smoke gate (added 2026-07-24): compiling is not launching.** `scripts/ios-launch-smoke.sh` fresh-installs the built app on a simulator, cold-launches, asserts the process survives 10s, screenshots, then warm-relaunches and asserts again; on failure it dumps the `GarageControl-*.ips` crash report and exits non-zero. It runs in `validate-ios.sh` + `ios-ci.yml` (Debug) and — the actual TestFlight gate — in `release-ios.yml` against a **Release-config simulator build BEFORE the archive step**, so a launch-crashing build aborts the release before anything reaches App Store Connect. The release lane runs it TWICE: newest runtime + an **iOS 16.4 (deployment-target) leg** — the ios/7 crash (below) reproduced ONLY on iOS 16.x, invisible on newer runtimes. Known residual gaps (signed-in-only, device-only, upgrade-state crashes): [`docs/IOS_RELEASE_SETUP.md`](docs/IOS_RELEASE_SETUP.md) § Launch smoke gate.
- **`self!` is BANNED in iosApp Swift; TestFlight crash logs are CLI-fetchable.** Empirical (ios/7, fixed in 0.1.1/#1123): `HomeViewModelWrapper.init` registered `for await` Tasks as `[weak self]` + `self!` — `StateObject(wrappedValue:)` re-evaluates its autoclosure on every view init but keeps only the first object, a discarded wrapper's Task ran after dealloc, and the force-unwrap crashed EVERY launch on iOS 16.3.1 (iPhone X) while iOS 18/26 sims stayed green. Safe pattern: `guard let stream = self?.<flow> else { return }` + `self?.` per iteration. Enforced by `scripts/check-ios-self-force-unwrap.sh` (validate-ios.sh step 1 + ios-ci.yml). To pull tester-shared TestFlight crash logs from the command line: `gh workflow run ios-crash-feedback.yml` (uses the CI-held ASC key; how this crash was root-caused).
- **CI signs from a STORED cert, not cloud signing (landed 2026-08-01 in #1191; the first release it actually signed was `ios/14`, 2026-08-18).** `-allowProvisioningUpdates` mints an Apple Distribution certificate on demand, and Apple caps those at a small number per account (commonly two; the portal is the authority) — so a job that creates one per release eventually fails every release at archive with `Your account has reached the maximum number of certificates` (plus a knock-on `No profiles for 'com.chriscartland.garage' were found`, which is Xcode falling back to Development signing). **Nothing reaches App Store Connect when this fires** — export/upload are skipped and the build number stays free (empirical: after `ios/12` AND `ios/13` both failed, the CI pre-flight still reported `Highest existing build on App Store Connect: 11`). Re-running the same tag only helps when the fix is **outside the repo** (a secret, a portal action): a tag-triggered workflow runs the workflow file **from the tagged commit**, so any fix to `release-ios.yml` itself needs a NEW tag. The cert + profile now live in `IOS_DIST_CERT_P12_BASE64` / `IOS_DIST_CERT_PASSWORD` / `IOS_PROVISIONING_PROFILE_BASE64` and are imported into a throwaway keychain. A cert minted by cloud signing **cannot be exported later** — its private key died with the runner — so recovering from the cap means generating a fresh cert locally. **Try creating it BEFORE revoking anything**: cloud signing's managed certs sit in their own bucket, and on 2026-08-18 the portal listed exactly one `Distribution Managed` cert yet still accepted a brand-new manual *Apple Distribution* cert with no revocation at all. Full runbook incl. expiry: [`docs/IOS_RELEASE_SETUP.md`](docs/IOS_RELEASE_SETUP.md) § "Signing: stored cert". The ASC API key is still required (the export step uploads with it); its Admin role was needed for cloud signing and is left in place — an App Manager key archived fine but failed export with `Cloud signing permission error` (empirical: `ios/1` attempt 1).
- **`PROVISIONING_PROFILE_SPECIFIER` must NEVER go on the `xcodebuild` command line (`ios/13`, fixed in #1226).** Command-line build settings apply to **every target in the build graph**, so all ~20 Swift Package dependencies inherit the profile and the archive dies ~20 times over with `Firebase_FirebaseAuth does not support provisioning profiles, but provisioning profile "…" has been manually specified`. It is the **only** signing setting the packages reject — `CODE_SIGN_STYLE` / `DEVELOPMENT_TEAM` / `CODE_SIGN_IDENTITY` are fine there, and the command line is the only level that outranks the `CODE_SIGN_STYLE: Automatic` baked into `project.yml`. So the specifier alone is written to a gitignored `MobileGarage/iosApp/Signing.local.xcconfig`, pulled in by an optional `#include?` from `Secrets.xcconfig` (same pattern as `Secrets.local.xcconfig`) — a **target** xcconfig is read only by that target. **Moving all four into the xcconfig does NOT work:** Xcode precedence puts target `.pbxproj` settings **above** the target xcconfig, so `Automatic` wins while the specifier still lands and the archive fails with `conflicting provisioning settings` (verified locally before #1226 was pushed). The file being absent locally is what keeps a local Xcode build on Automatic signing, needing no distribution profile.
- **A local archive cannot verify CI's signing.** CI imports the `.p12` into a throwaway keychain and runs `security set-key-partition-list -S apple-tool:,apple:,codesign:`; a local run uses your **login** keychain, a different mechanism. Driving `codesign` from a headless shell against the login keychain fails with `errSecInternalComponent` — an unanswerable GUI authorization prompt — which says nothing about CI either way. Local archives are still worth running to catch *configuration* errors (they caught both #1226 bugs), but do not try to close a keychain-path question locally, and do not loosen the login keychain's ACLs to try.
- **Deploy-capable creds live ONLY in GitHub Actions secrets** (`APP_STORE_CONNECT_KEY_ID` / `_ISSUER_ID` / `_KEY_P8`, `GARAGE_SERVER_CONFIG_KEY`) — never on a dev machine, by design. So pushing an `ios/N` tag → the workflow is the single credentialed deploy path. `release-ios.sh --check` is **git-tags-only advisory**; the **CI pre-flight** (`scripts/asc-latest-build.rb`, runs first, fails fast + loud) is the authoritative build-number gate. Do not re-introduce a local App Store Connect key.
- **App Store Connect silently renumbers a duplicate build** instead of failing (that's how `ios/1` landed as build 2 after a manual upload took build 1). The CI pre-flight aborts on a collision with the exact number to use. Tag N and the build number can drift when a build is uploaded out-of-band; `--confirm-tag` accepts any strictly-higher `ios/N` (skip-ahead) so you can push the number CI names. **Never write the next release tag into any doc — look it up when releasing** (it drifts the moment a release lands): `git tag -l 'ios/*'` for the highest git tag, and the CI pre-flight (`scripts/asc-latest-build.rb`) is authoritative for the next free build number on App Store Connect, which can run *ahead* of the git tags when a build was uploaded out-of-band or a release failed after archiving. Same rule for `android/N` and `server/N` — the release scripts' `--check` computes the next tag from git; don't hardcode it.
- **The iOS app icon must be a 1024×1024 OPAQUE (no-alpha) PNG** or App Store Connect rejects the upload (missing 1024 marketing icon / `CFBundleIconName`). Committed at `iosApp/GarageControl/Assets.xcassets/AppIcon.appiconset/`; rendered from `distribution/playstore/src/icon.svg` (`qlmanage` → flatten opaque). Verify with `actool` (command in the runbook).
- **TestFlight can't target the simulator** (the artifact is device-arch/device-signed; no TestFlight app on the simulator). Simulator testing = Xcode ⌘R directly; real push *delivery* needs a physical device.
- **CI must build with the iOS 26 SDK (Xcode 26+) — both iOS workflows pin `runs-on: macos-26`.** As of mid-2026 Apple rejects any App Store Connect upload not built with the iOS 26 SDK; `macos-latest` still resolved to Xcode 16.4 / iOS 18.5 SDK on 2026-07-01 and the `ios/3` upload was rejected at validation (*"must be built with the iOS 26 SDK"*) — the archive + cloud-sign steps passed, so it's a pure infra gate, not a code issue. `release-ios.yml` + `ios-ci.yml` pin `macos-26` (GA since 2026-02-26, default Xcode 26.5); the archive step logs `xcodebuild -version`. `ios/1` (2026-06-30) predated this enforcement. Full note: [`docs/IOS_RELEASE_SETUP.md`](docs/IOS_RELEASE_SETUP.md) § "Runner Xcode / iOS 26 SDK requirement".

### Releasing Wear OS

The Wear app (`MobileGarage/wearApp/`) shares the phone's applicationId and ships on the **Wear form-factor track of the same Play listing**. Canonical runbook (architecture, release lane, Play Console bootstrap, follow-ups): [`docs/WEAR_OS.md`](docs/WEAR_OS.md).

**The watch has TWO voice surfaces, and which is which is carried by TYPES, not by configuration (0.6.0+).** The mic on the door screen is **LIVE** — `WearLiveVoiceViewModel` drives the shared `VoiceCommandController` / classifier / gate against `RemoteButtonVoiceCommandEnvironment`, so a committed command presses the real garage button. Settings → Simulated voice is a **rehearsal** — `WearSimulatedVoiceViewModel`, same loop, `SimulatedVoiceCommandEnvironment`, no route to the button at all. (Until 0.5.4 the watch had only the simulated one; promoting it was the maintainer's explicit call on 2026-07-31, which is the bar for changing this again in either direction.)

The safety design is that **there is no `VoiceCommandEnvironment` binding in the Wear graph**. Each surface constructs its own environment literally, so the two are separated by a declared type at the call site rather than by a provider anyone could re-bind. Do **not** collapse them back into one ViewModel taking an environment — that trades a compile-time guarantee for a convention. Five tests enforce it: `WearSimulatedVoiceViewModelTest.cannotReachTheRealRemoteButton` (constructor reflection — note it also forbids `ObserveDoorEventsUseCase`, so the rehearsal cannot even be *gated* on the real door), `.theLiveSurfaceDoesReachTheRealRemoteButton` (the inverse — without it, deleting voice control outright would make every safety test pass more comfortably), `WearComponentGraphTest.theTwoVoiceSurfacesAreDistinctTypes`, `VoiceStringsTest` (every action-describing state must be worded per surface), and `:usecase`'s `SimulatedVoiceCommandEnvironmentTest`.

**One confirm gesture, two ways to start it (0.6.1).** Holding the door and speaking to the mic are the SAME interaction from the moment the ring starts sweeping: same 2s duration, same midpoint cue, same `ConfirmRing`, same haptic constant at every point of the journey (`GESTURE_START` / `CLOCK_TICK` / `CONFIRM`×2 / `GESTURE_END` / `REJECT`), and **cancellable right up to the moment the ring closes** — the midpoint buzz is pacing, not a point of no return. Both durations derive from `WearConfirmTiming.RING_JOURNEY_MILLIS`; **do not write the number twice** (they drifted to 2s vs 3s from 0.3.0 until 0.6.1, which made the ring mean two different things). `WearConfirmParityTest` pins the clock and every cue pair, plus a positive control that Committed and Refused still feel *different* — otherwise a `constantFor` returning one value for everything would pass the whole parity suite vacuously. The one deliberate asymmetry is the input: a hold is sustained so abandoning it is passive (lift your finger), voice is fired so abandoning it is active (tap anywhere). Full table: [`docs/WEAR_OS.md`](docs/WEAR_OS.md) § "One ring, two ways to start it".

**Haptics are the interface when the wrist is down (0.6.2).** Every tap AND every outcome on the voice surface has its own cue, and no two share a constant — `VoiceListening` (VIRTUAL_KEY, mic opened) / `VoiceArmed` / `VoiceHalfway` / `VoiceCommitted`×2 / `VoiceAborted` (a countdown **or** a live mic that you stopped) / `VoiceRefused`. Weight tracks stakes. `WearConfirmParityTest.everyVoiceMomentHasItsOwnFeel` fails on any collision. **The failure mode to design against is silence, not a wrong buzz**: a tap that registered nothing feels exactly like one that registered and misfired, and the user's instinct is to tap again — wrong on a surface that opens a garage. Two such silences existed until 0.6.2 (opening the mic, and cancelling before speaking). An outcome *expiring by itself* stays silent on both surfaces, deliberately: the user did nothing, and a buzz there would fire after every refusal.

**Leaves are moments, not places (0.6.2).** `WearDestination.survivesBackgrounding` is true only for `Home`; leaving the app dismisses any voice leaf, and the destination is `remember` (not `rememberSaveable`) so process death cannot restore one. *Arriving* on the voice screen is what opens the microphone, so a restored leaf is an inert screen the app reopens onto.

**The simulation announces itself four independent ways**, because one is missable on a wrist: a persistent `SIMULATION` marker present in every state (including the listening takeover that hides everything else), an **azure ring** instead of the real one's white (`WearRingColors.simulated` — hue is what reads while the ring is moving and the words are not), conditional wording throughout ("Would open the door", ending in "Nothing was sent"), and a door line labelled "Demo door". Signals 1 and 2 are the ones that work without reading, which is the case that matters. Keep the rehearsal **in Settings**, never beside the door: the mic on the door screen is the real control, so the pretend one must not sit where a hand reaching for it might land. Design: [`docs/WEAR_OS.md`](docs/WEAR_OS.md) § Voice.

Use `./scripts/release-wear.sh` — same `--check` → `--confirm-tag wear/N` model as the other lanes (validation marker + `wearApp/CHANGELOG.md` gate; versionName in `wearApp/version.properties`). Tag `wear/N` builds versionCode `1000000 + N` (offset keeps Wear codes disjoint from the phone's `android/N` codes in the shared package). The guardrails hook blocks pushing `wear/N` refs outside the script.

**Bootstrap COMPLETE (2026-07-22):** `wear/2` (0.1.1, versionCode 1000002) deployed to the Play **`wear:internal`** track fully automatically (`wear/1` was the one-time manual upload; the track identifier was confirmed via the `play-track-snapshot` log and `WEAR_PLAY_UPLOAD_ENABLED` is `true`). Watch visibility additionally required the app-level **Wear OS opt-in + Google quality review** (Advanced settings → Form factors) — approved 2026-07-22, after which 0.1.1 installed from Play on a physical Pixel Watch 4; see `docs/WEAR_OS.md` for the full gate description. `release-wear.yml` also always uploads the signed AAB as a **1-day CI artifact** (an expired artifact is regenerated by re-running the same tag's workflow run — no new tag needed), and sends `distribution/wear-whatsnew/` as release notes. The snapshot resolver maps wear versionCodes (1000000+N) back to `wear/N` tags. Wear release builds have **R8 minification deliberately OFF** (ADR-020 keep rules not yet ported/verified for Wear) — fine for internal testing, revisit before wider rollout.

## Documentation

Detailed project documentation lives in `MobileGarage/docs/`:

- [Architecture](MobileGarage/docs/ARCHITECTURE.md) — System overview, Android layers, data flows, DI, state management
- [Decisions](MobileGarage/docs/DECISIONS.md) — Architectural decision records (ADRs): server-centric design, tech stack, testing philosophy, migration targets
...
- [Dependency Upgrades](docs/DEPENDENCY_UPGRADES.md) — Sequencing playbook for multi-PR upgrades: 4-bucket framework (docs / pre-submit / runtime / deploy), per-class risk register, operational gotchas, and the "stay on Node 22" decision with tripwire to revisit. Read this before starting any dependency-upgrade sweep.
  - **Dependabot posture (2026-06-29):** configured **minor/patch only** (majors are `ignore`d — handle them by hand via this playbook; **don't merge a dependency major to clear the backlog**). **A red Android-side Dependabot PR is almost always a secret artifact, not a real break:** GitHub withholds repo Actions secrets from Dependabot-triggered runs, so the `google-services.json` decrypt no-ops and every Android job fails at `processGoogleServices`. CI handles it (`.github/actions/setup-android` drops in a committed placeholder `google-services.json` when there's no encrypt key — since #1049 the placeholder also carries a `com.chriscartland.garage.benchmark` client so `processBenchmarkGoogleServices` passes too; the **Build Release AAB** job — which also needs the real signing keystore — is skipped on Dependabot runs, and the `if: always()` `Android CI Complete` gate tolerates the skip). So: triage by whether CI is *genuinely* red (real breakage) vs. the secret artifact; enable auto-merge on green minor/patch PRs (`gh pr merge --auto` errors if the PR is already mergeable — use a plain `--squash --delete-branch` to merge it now). npm PRs are unaffected (Firebase CI needs no Android secrets). iOS CI is **not** made Dependabot-resilient (non-required, so it doesn't block).
  - **A GREEN Dependabot PR can still break iOS main — the KMP KLIB-ABI trap (2026-07-05, #1033 → fix-forward #1053).** Any KMP dep consumed from `commonMain` whose new version ships iOS KLIBs built with Kotlin 2.2+ compiles fine on the Android/JVM side (every REQUIRED check passes, auto-merge fires) but fails `:*:compileKotlinIosSimulatorArm64` with `KLIB resolver ... incompatible ABI version` — only the non-required iOS CI goes red, post-merge. Empirical: kermit 2.1.0 (Kotlin 2.2.0), androidx.datastore 1.2.1 (2.2.20), androidx.sqlite 2.7.0 (2.3.20) — all three now pinned in `libs.versions.toml` + `dependabot.yml` ignores; unpin with the Kotlin 2.2+ bump. **Before merging a group bump that touches ANY commonMain dep, run `MobileGarage/gradlew -p MobileGarage :iosFramework:iosSimulatorArm64Test` locally** — CI stops at the first broken module, so one local run can reveal more casualties than the CI log shows.
...
- **Single-card "relaxed-A" — ENABLED + LIVE in production since 2026-07-03; the go-forward instead of Phase 2.** Rather than Phase 2's risky "move the warning to app-built data-only" (which drags the primary state-sync onto a new topic → silent-dark risk), the relaxed-A design gets the never-woken single card by keeping the warning OS-rendered and just (a) tagging it and (b) making the resolved a combined notification+data message on `door_open_v2-` sharing that tag, so the OS collapses them. Two live flags (both **flipped `true` 2026-07-03** via `set-config-flag.sh`; shipped dark in `server/32`/`server/33`; verify with `--list`): **`warningReplaceTagEnabled`** (tags the `door_open-` warning; the ONLY old-app-observable change — stacking→replace-to-latest) + **`resolvedNotificationPayloadEnabled`** (combined resolved). No Android release was needed to activate — the installed `2.20.2` already handles both (foreground app-built / background OS-rendered). Revert = flip either flag `false` (instant, no redeploy). A multi-agent impossibility proof (strict zero-old-app-change makes the never-woken single card impossible) + the relaxed-A design + the adversarial verification (no old-app degradation) live in [`docs/RESOLVED_NOTIFICATION_NO_COMPROMISE.md`](docs/RESOLVED_NOTIFICATION_NO_COMPROMISE.md). **Device gate: PASS on Pixel (2026-07-02)** via the sandbox `--mode notification|combined` — a same-tag warning replace re-alerts (→ no old-app degradation), and warning→combined-resolved collapses to one card. Gated on Pixel only — if a non-Pixel OEM (Samsung/Xiaomi) later becomes a primary notification device, re-run the re-alert gate there (a non-conformant ROM could silently update a warning without re-alerting). Enabling `resolvedNotificationPayloadEnabled` has a known accepted tradeoff — see next bullet.
- **Android 8+ (Oreo): CHANNEL importance overrides per-notification priority — you cannot quiet an OS-rendered notification with `notification_priority` / `setPriority`.** The Pixel gate confirmed a `PRIORITY_LOW` combined-resolved still heads-up/buzzes because it lands on the HIGH "Garage door" channel (via the manifest `default_notification_channel_id`). So the combined resolved uses **HIGH** delivery priority (for reliable never-woken Doze arrival) and the buzzing all-clear is **accepted**; a silent all-clear would need a **dedicated lower-importance channel** (app-created + `channel_id` on the payload), deferred. General rule: to change heads-up/sound behavior for an OS-rendered FCM notification, change the CHANNEL, not the priority field. The sandbox (`scripts/send-test-notification.sh --mode notification|combined --tag ...`) is the device-gate tool for any same-tag replace / re-alert / cross-topic-collapse question.

### iOS simulator testing — the remote button is the only physical action; never tap it

The iOS app (`MobileGarage/iosApp/`) shares the Android business logic via `:iosFramework`, so the **remote garage button is the only path that operates the real door.** It is well-guarded — verified 2026-06-24:
...
The local toolchain works (Xcode 26.5 + xcodegen + simulators), so iOS changes can be verified locally before pushing: `cd`-free `xcodegen generate --spec MobileGarage/iosApp/project.yml --project MobileGarage/iosApp` then `xcodebuild -project MobileGarage/iosApp/GarageControl.xcodeproj -scheme GarageControl -sdk iphonesimulator -destination 'generic/platform=iOS Simulator' build CODE_SIGNING_ALLOWED=NO`, then `simctl install`/`launch`/`io screenshot`. A pre-build Run Script rebuilds `shared.framework` via Gradle (slow first time; pass `OVERRIDE_KOTLIN_BUILD_IDE_SUPPORTED=YES` to skip it when only Swift changed). Reading SKIE's emitted `…/build/skie/.../*.swift` + the framework `Headers/shared.h` gives the exact Swift conformance contract (this is how the bridge `__`-prefixed-async + `SkieSwiftOptionalFlow` shapes were pinned).

**Runtime verification on the simulator has three traps that read as "the feature is broken" when the code is fine** (each cost real time on 2026-08-24 and together produced a false bug report that was only retracted after side-by-side builds): (1) a **headless simulator never activates the scene** — `simctl boot` without Simulator.app open leaves the app `.inactive` forever, so `scenePhase`/lifecycle-driven behavior never starts; open the Simulator UI before trusting a negative result. (2) On a fresh install the **notification-permission alert holds the scene inactive** — visibility-gated behavior genuinely hasn't started while it's up; pre-grant with `applesimutils --booted --bundle com.chriscartland.garage --setPermissions notifications=YES`. (3) `log stream` needs **`--level debug`** or Kermit's `Logger.d` output is silently absent. Full runbook + expected log sequences: [`docs/DOOR_UPDATE_STRATEGY.md`](docs/DOOR_UPDATE_STRATEGY.md) § Verification. The general lesson: unit tests that INJECT a platform signal (e.g. `AppVisibilityState`) prove the logic given the signal and say nothing about whether the platform emits it — that gap closes only by running the app.

Three traps for iOS PRs:
- **`iosFramework` Kotlin IS covered by the Android required check.** `Android Checks / Formatting & Static Analysis` runs `:iosFramework:spotlessApply --check`, so an "iOS-only" PR that edits `iosFramework/src/**/*.kt` can fail a **required** Android gate on formatting. Run `./gradlew -p MobileGarage :iosFramework:spotlessApply` (or full `validate.sh`) before pushing iOS-Kotlin changes. #915 was blocked on exactly this (a multi-line fun that spotless wanted as a one-liner).
- **iOS CI (`Build iOS app + framework test`) is NOT a required check.** With auto-merge on, a PR merges as soon as the *required* (Android/Firebase/Script/Doc) checks pass — iOS CI often finishes *after* the merge. #912/#913/#915/#936/#937 all merged before iOS CI completed (all came back green because each was verified locally first). Mitigation: verify locally, and watch the iOS CI run to catch a post-merge red and fix-forward. Same root cause as the `NativeComponent` three-DI-component trap above.
  - **Confirm the iOS-CI outcome with `gh run view <id> --json status,conclusion`, NOT the `gh run watch --exit-status` exit code.** Empirical (#961): `gh run watch --exit-status` returned **exit 0** while the run had actually concluded **`failure`** — the watch exit code is unreliable for pass/fail. Always read back `conclusion` explicitly (same spirit as "trust validate.sh's printed markers, not the exit code").
  - **`Run iosFramework simulator tests` flakes on Gradle daemon startup.** A red on that step whose log shows `The daemon has terminated unexpectedly on startup attempt #1` + `A build operation failed` on `:iosFramework:iosSimulatorArm64Test` — with **no** `Test Case … failed` / no assertion failure — is an **infra flake**, not a regression (#961 hit this on a pure-Swift PR that can't even touch the Kotlin tests). Confirm `main` is healthy by running it locally: `MobileGarage/gradlew -p MobileGarage :iosFramework:iosSimulatorArm64Test --rerun-tasks` (`NativeComponentTest`, 40 tests). Don't fix-forward a flake; verify-then-move-on. **Second flake signature, same step, same protocol** (#1209, 2026-08-15): `Multiple entries with same key: N=…TestOutputStore$Index@…` + `BUILD FAILED` with no compile error (`e:`) and no failed test case — Gradle test-result-store corruption on the runner, not a regression. The local `--rerun-tasks` run is the health check either way.
- **SKIE does not bridge Kotlin default arguments.** A shared `fun f(x, y, z = default)` consumed from Swift fails the **iOS compile** with *"missing argument for parameter 'z'"* — Swift sees only the full-arity signature. Provide an explicit overload (`fun f(x, y) = f(x, y, default)`) instead of relying on the Kotlin default; the Swift call then resolves to the short overload. Canonical: `CheckInStatusMapper.forCheckIn`'s two-arg overload (#936, ADR-031 P5). This is caught **only** by the CI-exact iOS build (`generate-ios-screenshots.sh` / iOS CI), never `validate.sh` (Android-only, and Kotlin defaults compile fine for Android) — so build iOS locally after adding/changing any shared API consumed from Swift. (Other SKIE bridge shapes: sealed types → `onEnum(of:)` with camelCase cases, `data object` → no-payload case; `StateFlow<Long>.value` → `KotlinLong.int64Value`; keep date-library types off the public surface. A Kotlin `object`/`companion object` `const val` keeps its name **verbatim** in Swift — UPPER_SNAKE is *not* camelCased, so `GarageDoorGeometry.shared.VP` / `AppLinks.shared.PRIVACY_POLICY_URL`; a shared `val xs: List<Float>` bridges to `[KotlinFloat]`, mapped via `.map { CGFloat(truncating: $0) }`. A plain Kotlin `enum class` bridges to a Swift `enum` with **camelCase** cases (`NONE`→`.none`, `OPENING_TOO_LONG`→`.openingTooLong`), switched **exhaustively with no `default`** — unlike a `sealed` type's `onEnum(of:)` (canonical: `DoorOverlayKind`, #956). Scalar primitives **unbox**: a `fun(): Float` / scalar `const val Float`/`Long`/`Int` bridges to an unboxed Swift `Float`/`Int64`/`Int32` (wrap `CGFloat(...)` / `Int(...)` / `Double(...)`) — contrast `List<Float>` above, whose *elements* box to `KotlinFloat`. To go the **other way** — pass a Swift `Int64?` *into* a Kotlin `Long?` parameter (the reverse of `.int64Value` reading) — **construct the box**: `KotlinLong(value: $0)`, e.g. `lastChangeTimeSeconds.map { KotlinLong(value: $0) }` for a nullable. Canonical: door-geometry #952, palette #954 (`const val Long`→`Int64`), animation-spec #956 (`enum class` + `fun(): Float`), door-trajectory #959/#960 (`const val Int`→`Int32` for `ANIMATION_DURATION_SECONDS`; `KotlinLong(value:)` to build a `DoorMotionKey`'s `Long?` from Swift `Int64?`).)

  **SwiftUI `.onChange(of:)` handlers must use the closure's `newValue`, never the captured struct property.** The invoked closure can be the one registered by a *previous* body evaluation, so `{ _ in doThing(with: self.someProp) }` acts on the OLD struct copy's value — `@State` that should track a prop silently sticks. Empirical: `AnimatedDoorCanvas` (#1055) — every fresh install rendered the Home door half-open ("Closed" label, color/overlay correct, offset frozen) until relaunch, and live transitions animated one state behind. Invisible to the snapshot gallery (static-mode only); the CLI verification is the fresh-install simulator repro (uninstall → install → launch → timed screenshots). Headless sim interaction (idb taps, applesimutils permission pre-grant) + the Tailscale/HTTP-3 sim-network gotcha are documented in [`MobileGarage/distribution/appstore/LISTING.md`](MobileGarage/distribution/appstore/LISTING.md) § screenshots.

  **Non-integer Tier-1 constant hoists shift iOS snapshots by a benign sub-pixel edge.** Geometry/palette hoists were all integer-valued (exactly representable) → bit-identical iOS regen. A hoisted value *not* exactly representable in IEEE-754 (door offsets `-0.65f`/`-0.20f`/`-0.35f`) renders a **single ~1px-tall anti-aliased edge row** of difference per affected snapshot, because the shared Kotlin `Float` widens to `CGFloat` differently than iOS's old Swift `Double` literal. Inspect the diff `bbox`: a **1-D horizontal line** (`(x0, y, x1, y+1)`) is the benign float-precision signature — accept the regen (it actually *improves* parity, since Android Compose also renders offsets as `Float`); a **2-D region** is a real render change — investigate. Full exception in [`MobileGarage/docs/UI_FIDELITY_TIERS.md`](MobileGarage/docs/UI_FIDELITY_TIERS.md) § verified-no-op recipe step 4. Empirical: `DoorAnimation` hoist #956 (2 snapshots, ≤0.02% of pixels).

### iOS snapshot gallery (Prefire + swift-snapshot-testing, regenerate-don't-assert)

iOS has a browsable visual reference of every SwiftUI `#Preview` — the analog of the Android screenshot gallery — covering the door component + **all 5 screens** (Home / History / Profile / Functions / Diagnostics). Full design: [ADR-030](MobileGarage/docs/DECISIONS.md); how-to: [`MobileGarage/iosApp/SnapshotTests/README.md`](MobileGarage/iosApp/SnapshotTests/README.md). Regenerate with `./scripts/generate-ios-screenshots.sh`; browse `MobileGarage/iosApp/SnapshotTests/SCREENSHOT_GALLERY.md`. Shipped across #920 (infra) → #921/#922/#924 (screens).
...
- **When adding/refactoring an iOS view, add a `#Preview`** so it lands in the gallery — and when CHANGING a view an existing `#Preview` already renders, regenerate in the same PR: the committed reference PNG is stale the moment the view changes. Empirical: #1229 added the door-updates picker to `SettingsContentView` without a regen; #1233 fixed it (all 31 re-recorded, exactly one PNG changed — which is also the determinism check). Screens take a live `NativeComponent`, so split each into a pure `*ContentView` (plain values + action closures) + fixture `#Preview`s — the `HomeContentView`/`HistoryContentView` pattern.
- **`#Preview` bodies are embedded verbatim** into the generated test (`@testable import GarageControl`), so a body may only reference `internal`+ symbols — never a `private` file-scope helper (inline such fixtures). `traits:` overloads are iOS 17+ (deployment target is 16) — use plain `#Preview("Name") { }`.
...
- **The regen deletes-then-re-records ALL snapshots — never read `git status` (or commit) mid-run.** `generate-ios-screenshots.sh` removes every `__Snapshots__/*.png` and re-records them in one pass, so a `git status` taken *while it runs* shows **all** snapshots as deleted (`D`). That's transient: wait for the run to finish (it prints `Recorded N reference PNG(s).`), then check — the steady state restores every PNG with only the genuinely-changed ones `M`. Committing mid-run would stage spurious deletions. (Empirical 2026-06-29: peeked mid-run, saw 21 deletions, waited, all restored; only the 4 intended Home PNGs changed.)
- **An invalid `Image(systemName:)` SF Symbol renders BLANK, not a build error** — the compiler accepts any string. So the snapshot regen is the verification: after changing any glyph, resize + Read the regenerated PNG (`sips -Z 800` first, per the image-size hook) and confirm the icon actually renders. Canonical: the device-availability pills swapped to `wifi`/`wifi.slash` (#978) as the iOS visual equivalent of Android Material `Sensors`/`SensorsOff`; verified the pill wasn't blank before committing. **Record cross-platform icon-name pairings in the consuming view's doc comment** (e.g. `DeviceCheckInPill`) since the two icon systems can't share a literal glyph.
...
### Shared decides, platform words it (ADR-035) + iOS localization

**The rule (ADR-035, adopted #1158).** The shared KMP layer states *what is true* as typed values; each platform says it *in words*. The test to apply: **does shared logic decide which variant the user sees?** If yes, the choice belongs in shared as a small enum / sealed type and the wording belongs in `strings.xml` / `Localizable.xcstrings`. If no — static text with no branching — it is a plain platform string and KMP is not involved at all. There is deliberately **no** global string enum; scope each type to its own bit of UI.

Shared decision types live in `presentation-model/` with `commonTest` coverage: `ElapsedDuration`/`SinceStatusMapper`, `DoorWarning`, `RemoteButtonDiagram` (#1160), `SnoozeRowStatus` (#1161), `DoorHeadline` + `PermissionNagLine` (#1162), `StateSpanDuration`/`TransitSpanDuration` (#1163). `ButtonOfflineAge` is in `usecase/`.

**Corollary — name the shared type after the CONCEPT, never after one platform's word for it (added #1188).** ADR-035 says shared decides and the platform words it. That only works if the shared name doesn't answer the wording question prematurely. iOS shipped an About row labelled **"Package"** — Android's term for `applicationId`; iOS has a *bundle identifier* and every Apple surface calls it the **Bundle ID**. It got there by mirroring the Android screen row-for-row, and because both values are `com.chriscartland.garage` nothing *looked* wrong. The fix is `AppBuildFact.STORE_IDENTIFIER` (in `presentation-model/`), deliberately using neither vocabulary: a case called `PACKAGE` tells an iOS implementer the question is settled, when the whole point is that it isn't. Each case's KDoc records the native API **and the correct user-facing label on both platforms**, so the answer is in front of whoever adds the next one. `AppBuildFactTest` enforces the naming rule (no `PACKAGE`/`BUNDLE`/`CFBUNDLE`/`VERSION_CODE`/`MARKETING` in a case name) with a positive control so a typo in the word list can't make it vacuous. **Terminology pairs worth knowing:** Android *package name* = iOS *Bundle ID*; Android *version code* = iOS *build number* (`CFBundleVersion`, a dotted **string**, not a Long); Android *version name* = iOS *marketing version* (`CFBundleShortVersionString`).

**Verify cross-platform exhaustiveness empirically, with a probe.** A shared Kotlin `enum` consumed by both platforms is only a parity guarantee if BOTH compilers actually enforce it. Don't assume — add a throwaway case, confirm Android fails (`'when' expression must be exhaustive`) AND iOS fails (`switch must be exhaustive`), then delete it. Done for `AppBuildFact` (#1188); both fired. The iOS half needs the **CI-exact** build (no `OVERRIDE_KOTLIN_BUILD_IDE_SUPPORTED`, `-destination 'generic/platform=iOS Simulator'`) — `validate.sh` is Android-only and would report green. Note SKIE bridges a Kotlin enum as a Swift enum conforming to `CaseIterable`, so iteration is **`allCases`**, not `.entries`.

**Non-violations — never "fix" these into shared strings:** FCM topic names (`door_open-`, `door_open_v2-`), `AppLoggerKeys`, JSON `@SerialName` keys, Ktor paths, Room table/column names, DataStore keys, `SnoozeDurationServerOption`'s `"0h".."12h"`, Kermit log bodies, `require` messages, and preview fixtures. Those are protocol or diagnostics, not user-visible copy.

**A refactor of this kind should produce ZERO snapshot diffs.** Every one of #1160–#1163 did. A changed PNG means behavior moved, not just representation — stop and find out why.

**iOS specifics — see [`MobileGarage/docs/IOS_LOCALIZATION.md`](MobileGarage/docs/IOS_LOCALIZATION.md) before touching iOS strings.** The load-bearing points:

- **SwiftUI extracts a string only where it can see a `LocalizedStringResource`/`LocalizedStringKey`.** A literal that has been flattened to a `String` before reaching `Text` is **permanently unlocalizable, silently** — no warning, and the app looks perfect. Prefer `LocalizedStringResource` (iOS 16+, the deployment target) for any display text: wrapper `@Published` properties, view parameters, model computed properties.
- **`Text(cond ? stringVar : "literal")` kills the literal.** Type unification makes the whole ternary a `String`. Write `if`/`else` with two `Text`s.
- **Copy vs data is a real distinction** — `Core/DisplayText.swift`'s `.copy(LocalizedStringResource)` / `.data(String)`. User names, email addresses, version strings, and already-locale-formatted dates must render `Text(verbatim:)`; translating them is meaningless, and a value colliding with a catalog key would be silently rewritten. Do **not** make such a type `ExpressibleByStringLiteral` — literals would arrive as plain `String` through `init(stringLiteral:)` and never be extracted, which is the exact bug, hidden behind tidier call sites.
- **Verify with the catalog, never by eye — and run the sync IN THE SAME PR that adds iOS strings.** `./scripts/sync-ios-string-catalog.sh` then check the key is in `Localizable.xcstrings`. Nothing gates catalog freshness at PR time (the sync is manual), so a PR that adds `LocalizedStringResource` strings without running it merges green while the catalog silently misses the keys. Empirical: #1229 added the five door-updates picker strings; a health audit caught the gap a day later (#1232, 175→180 keys). A `validate-ios.sh` fence (fresh extraction diffed against the committed catalog) is the structural fix, not yet built. `xcodebuild` alone does **not** populate the catalog (that is an Xcode.app behavior), and `SWIFT_EMIT_LOC_STRINGS` defaults to `NO` in XcodeGen projects — without it nothing is extracted and the catalog sits empty looking installed and correct.
- **`scripts/check-ios-localizable-text.sh`** (validate-ios.sh step 2 + ios-ci.yml) fences the pattern from spreading, with a burn-down list at `MobileGarage/ios-localizable-text-exemptions.txt`. **It cannot tell `String` from `LocalizedStringResource`** — shell has no types — so it is a "do not spread" fence, not a per-file verdict, and the list does **not** empty out as files are fixed. `Text(verbatim:)` is the never-flagged escape hatch.
- **`git grep -E` uses POSIX ERE, where `\b` is NOT a word boundary.** A pattern containing it matches *nothing* and the check passes vacuously — same family as the Konsist `file.name` trap. Use `(^|[^A-Za-z0-9_.])`, and give any new grep-based check a scope-sanity guard that fails when the broad form matches zero files.
- **`Localizable.xcstrings` conflicts on every parallel PR that adds strings.** Regenerate, don't hand-merge: `git checkout --ours` it, finish the rebase, re-run the sync (it only ever adds), amend. Recipe in the doc.

### The settle window (arriving is not the same event as failing)

**Every screen gets five seconds to look current before the app is allowed to say anything is wrong.** `AppSettleWindow` (`:usecase`, ADR-015 manager, `SETTLE_WINDOW_MILLIS = 5_000`) opens a grace period each time the app becomes visible; `DataFreshness` (`presentation-model`) turns that plus what is known about the data into one of three verdicts, and every platform renders the same three.

- `FRESH` — normal.
- `SETTLING` — not confirmed current, but the app only just arrived. **Door grey and dim; not one word changed.**
- `STALE` — the doubt outlived the window. **Same picture, now with words.**

**Indicators only ACCUMULATE along that sequence** — a state never gets more colourful as the news gets worse. Two predicates carry it: `isMuted` (SETTLING + STALE, the greyed door) and `isSpoken` (STALE only, the single gate on every *worded* freshness indicator — the stale banner, the fetch-error banner, and the Retry hanging off them). The sub-rule for colour: **colour that ALARMS is gated on `isSpoken`; colour that merely withholds confidence is gated on `isMuted`.** That is why the check-in pill keeps its label ("23 min ago") throughout and only its red waits.

**The bug it fixes:** a warm start on a door whose check-in had aged past the 11-minute threshold went straight to the alarm presentation — banner plus Retry — for the second it took the return fetch to land. Every launch showed an error that was gone before the user could act on it, which teaches people to ignore the banner for the one time it is real.

Load-bearing details, each of which cost something to get right:

- **Five seconds of SCREEN TIME, counted — not five seconds of wall clock, timed.** This is the difference between a grace period and a silencer. The first implementation restarted a `delay(5_000)` on every return, so it only closed after five *uninterrupted* visible seconds — and nothing guarantees the app gets five uninterrupted seconds. On the watch `onVisible`/`onHidden` are `ON_START`/`ON_STOP`, so a user glancing for three seconds at a time (the normal way people use a watch) restarted the countdown on every glance and would **never** be told the garage was unreachable; on iOS a notification banner, Control Center or an incoming call all report `.inactive` and each dismissal bought another five seconds of silence. That turned "delay the warning" into "suppress it indefinitely" — the one thing the feature promised it could not do. The countdown now accumulates: leaving PAUSES it, returning RESUMES it, and only a return arriving after the window has already closed resets it to a fresh five seconds (which is exactly the warm start the grace is for). Scar test: `AppSettleWindowTest.screenTimeAccumulatesAcrossGlancesSoShortLooksStillReachStale`, verified to fail against the old design.
- **Keyed on VISIBILITY, not on a fetch.** One return may involve a cache read, a token refresh and a fetch; the user experiences one arrival and should get one grace period. `AppSettleWindow` collects `AppVisibilityState.visibility` with `collectLatest`, so the epoch (not the boolean) is what reacts to a return — a fast background/foreground round trip conflates to the same boolean and would otherwise never reopen the window. Scar test: `aReturnThatConflatesWithItsDepartureStillReopensTheWindow`.
- **`isSettling` starts `true`, and hiding is NOT an answer.** The platform's first `setVisible` lands after the first composition, so seeding `false` would let one frame of the full alarm through — the exact flash being removed. The collector therefore ignores non-visible values rather than closing the window on the starting `Visibility(false, 0)`, and that same early return is what makes a departure a pause rather than a rewind.
- **`AppSettleWindow` must be a singleton, and it must be STARTED.** It owns a `MutableStateFlow` *and* the collector job that drives it. Uncached, `AppStartup` starts one instance while the ViewModel collects a different, never-started one; never started, the flag sits at its seed `true`. Either way every worded freshness indicator is suppressed for the life of the process, silently. Pinned by an `assertSame` in all three component tests plus `AppStartupTest.onActivityCreated_startsTheSettleWindow` — the action-list assertion alone was not enough, because it passes if `start()` is deleted and only `actions.add` is left.
- **The dim lands on the door ART only, never on the text.** It first wrapped the whole card body, which dimmed the door's own warning chip to 55% — and STALE is indefinite, so the single most important sentence on the card was the one the feature permanently made hardest to read.
- **iOS greys the door by COLOUR, not with `.saturation(0)`.** The SwiftUI filter measured only ~70% desaturated in a recorded snapshot (a door pixel reading `spread=117` fresh came back at 35, not 0), which would have made "grey" mean something visibly different on iOS. `DoorPalette.desaturated` routes through the shared `FreshnessTint.luma` (Rec. 709) instead, so all three apps drain by exactly the same amount and the snapshot measures `spread=0`. The dim half stays a plain `.opacity`.
- **An empty cache escalates its HEADLINE, and that rule is shared.** `StatusHeadline` (`presentation-model`) is `Door(DoorHeadline) | Connecting | NoSignal`: with no door event, `Connecting…` becomes `No signal` once `isSpoken`. It exists because `DoorHeadline` cannot absorb it — an exhaustiveness test binds its six cases 1:1 to the nine `DoorPosition`s, and "we have heard nothing" is not a door position. Before it, Android and iOS said "Connecting…" **forever** on an empty cache and raised no banner either (`HomeAlertMapper` has arms for a stale check-in and a failed fetch, none for *never heard anything*), so the settle window's "then show more indicators" half simply did not happen on the phone; only Wear escalated, with its own local rule. **A reading we HAVE keeps being named even when spoken** — muted card, banner shown, headline still saying what the door was last known to be doing, because replacing a known position with "No signal" discards the last thing we know exactly when it matters most. Pinned by `aKnownDoorIsStillNamedWhenTheVerdictIsSpoken`.
- **Wear's bar for "not current" is `hasData` + `isFetchError`.** The poll loop's result is no longer discarded: a watch that succeeded once and failed every poll since kept `hasData` true for the life of the process, so the dial stayed fully saturated and confident over hours of failures — asserting a six-hour-old reading as current. The third input, `isCheckInStale`, is still absent for want of a clock on the watch (`CheckInStatusMapper.forCheckIn` needs a `now`), which is a known gap with a known fix rather than a design boundary. Wear's one worded escalation is the headline going `Connecting…` → `No signal`; the round mask has no room for a banner.
- **The `connecting` / `no_signal` and `HomeContentSettling` / `HomeContentSettledStale` fixture PAIRS are the design.** Review them together: same picture, a banner in exactly one. A regression that makes the muted look appear or disappear *between* the two is the thing to catch.

**Wear sign-in, same rule applied to identity.** `RelayFallbackAuthBridge` used to `emit(null)` before it had asked the phone anything; `FirebaseAuthRepository` maps null to `Unauthenticated` and the hero screen answers that with a Sign in button — so the watch offered sign-in on every launch, over an account the next emission was about to find. It now stays silent (leaving `AuthState.Unknown`, which the screen already words as "Checking sign-in") until the relay answers, bounded by `DEFAULT_UNRESOLVED_GRACE_MILLIS`, which *is* `SETTLE_WINDOW_MILLIS`. The grace emission races the request rather than cancelling it, so a late answer still signs the watch in: the bound can only offer a button early, never lose an account.
...
- **Why iOS was dark, and Phase 2's status: code complete, NOT deployed.** `EventFCM.ts` sent `data` + `topic` + `android` and nothing else — `apns: ApnsConfig` was commented out in `FirebaseServer/src/model/FCM.ts` since 2021. A data-only FCM message reaches an Apple device only with `apns.payload.aps['content-available'] = 1`. The whole iOS receive path (`AppDelegate.didReceiveRemoteNotification` → shared `FcmPayloadParser` → `ReceiveFcmDoorEventUseCase`) has been built and correct since #915; the server simply never addressed a message to it. **`getFCMDataFromEvent` now sends it** (`ApnsConfig`/`ApnsHeaders`/`ApnsPushType`/`ApnsPayload`/`Aps` added to `model/FCM.ts`, written in the FCM v1 REST API's own wire shape — matching the file's existing convention, confirmed necessary by reading how `firebase-admin` actually serializes `send()`), with 5 teeth-checked tests. **Deployment is deliberately withheld**: merging doesn't deploy Cloud Functions (only a tagged `release-firebase.sh` run does), and it is on the path that carries every Android device's door updates too, so the actual release — its own `server/N`, a changelog entry, and a check that Android still receives events afterward — is left for the maintainer. Full writeup: [`docs/DOOR_UPDATE_STRATEGY.md`](docs/DOOR_UPDATE_STRATEGY.md) § Phase 2.
- **A strategy decides WHEN to ask, never what the answer means.** Every implementation ends at `DoorRepository.fetchCurrentDoorEvent()`, or at push, which lands in the same repository. Cache, flows, and screens are identical whichever runs — that is what makes a runtime swap safe.
- **The platform reports, shared code decides.** `AppVisibilityState` is a **sink the platform writes** (`setVisible`), mirroring how door events already arrive (`FCMService` / `AppDelegate` call into shared code). No `expect`/`actual`, no bridge interface for lifecycle. Android counts started Activities in `GarageApplication` (so a rotation goes 1 → 2 → 1 and never reports a background round trip); iOS reports `scenePhase == .active` from `GarageControlApp`. It is also the test seam — there is no fake, tests construct the real object. **The value is `Visibility(isVisible, epoch)`, never a bare Boolean** — "the user came back" is an event, and an equality-conflated `StateFlow<Boolean>` drops it when the collector was suspended across the `false`/`true` pair (fast lock/unlock; iOS process suspension), leaving the return fetch silently unfired. The epoch bumps only on real `false → true`, so every return survives conflation and over-reporting stays free. Scar test: `aReturnThatConflatesWithTheDepartureStillFetchesImmediately` (fails against a Boolean).
- **Names are behaviors, never transports** (`PUSH`, not `FCM`). Push is FCM on Android and FCM→APNs on iOS; the transport is a platform fact and each platform words its own picker. Same rule as `AppBuildFact.STORE_IDENTIFIER` (§ "Shared decides, platform words it").
- **`PLATFORM_DEFAULT` is a real stored value, not a synonym for the current default.** Storing the resolved id would freeze whatever the default was the day the user first opened the picker — and iOS's default is expected to move in Phase 3, which is exactly the case it would break.
- **`currentDoorEvent` keeps `Cadence.PUSH` — do NOT relabel it `POLL`.** The annotation names what changes a node *behind the app's back*; app-initiated fetches have never changed it (`Cadence.USER_ACTION` reads "a user action **or an app-initiated fetch**", and `InitialDoorFetchManager` already writes this node the same way). Mechanically, `POLL` means a subscriber-held loop that gating can pause, so it would force `Sharing.Gated` (G4) on every derived node downstream to pause a loop no subscriber holds open. `AppVisibilityState` is likewise not a node: it drives *when* the app fetches, not any node's value, and it is a concrete class so the C1 flow sweep does not enumerate it.
- **Wear's loop has no failure backoff, and that is correct — do NOT add one.** It discards the fetch result and sleeps a fixed 10s, which would be a defect on the phone. On the watch it is bounded by construction: `onVisible()`/`onHidden()` are wired to `ON_START`/`ON_STOP`, there is no ambient or always-on mode, and `keepScreenOn` caps at 15s — so the loop lives only while someone is looking at the watch. Backoff would also be actively wrong there: the user is watching the screen now, and recovery is exactly the moment they want a fetch, not a sleep.
- **`PushDoorUpdateStrategy` deliberately does not own FCM topic subscription.** `FcmRegistrationManager` does, and those topics also carry button-health + resolved-notification traffic — swapping strategies must never silently unsubscribe a device from its notifications.
...
- **No `open class` for testability — use interface + `Default*` impl.** `open class X` exists in this codebase only when tests need to substitute behavior, which is the wrong direction: production code can accidentally extend it too, and the test fake is forced to call a real constructor with real deps. Convert to `interface X` + `class DefaultX : X` so test fakes implement the interface directly with no production constructor args (canonical: `RegisterFcmUseCase` after PR #669, `ReceiveFcmDoorEventUseCase`). **The exception is an `abstract class` that exists to SHARE implementation between concrete siblings that must not diverge** — not to let a test substitute anything. Canonical: `WearVoiceViewModel` (wear 0.6.0), whose two subclasses differ only in which door they act on; the subclass boundary is also what carries the safety property, since `WearSimulatedVoiceViewModel`'s constructor is asserted to have no route to the remote button. A single class taking the collaborator as a parameter could make no such promise — it would depend on the call site rather than the type. Don't "fix" that hierarchy into an interface.
...
- **UseCases that combine flows expose `StateFlow` via `stateIn(applicationScope, SharingStarted.Eagerly, initial)`.** A `combine(...)` result is naturally a cold `Flow` — first emission to a fresh subscriber is async and the consumer reads `Loading` for a frame on every fresh `NavBackStackEntry`. To deliver a synchronously-readable cached value, the UseCase wraps the combine with `stateIn` keyed on `applicationScope` (lives for app lifetime) with `Eagerly` (combine starts immediately and never stops). The provider must be `@Singleton` so the eager combine runs exactly once per process — multiple instances would each spin up their own collector with their own initial value, defeating the cache. `ComponentGraphTest` `assertSame` is the enforcement mechanism. Canonical: `ComputeButtonHealthDisplayUseCase` after PR #739. The `Eagerly` vs `WhileSubscribed` choice is keyed on **upstream cost**, not taste: `Eagerly` when the upstream is cheap in-memory `StateFlow`s (running the combine forever costs nothing); `WhileSubscribed(5_000)` only when collecting costs battery, IPC, or network — it pauses the expensive upstream while **retaining** the last value (`replayExpirationMillis` defaults to `Long.MAX_VALUE`, so it does NOT re-emit `initialValue` after the timeout; a returning subscriber reads the cached value synchronously). Canonical for the gated shape: `ObserveWatchAppStatusUseCase` (15s Play Services poll upstream). Never `WhileSubscribed` on a repository-owned `StateFlow`, where an emission landing in the dead window is lost forever. Full criterion + graph rules: `MobileGarage/docs/DATA_GRAPH_PLAN.md`.
