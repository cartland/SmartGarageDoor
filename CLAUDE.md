...
- **Edge-to-edge bottom inset propagates to scrollable leaves.** `Main.kt`'s Scaffold body wrappers consume only what they structurally handle: in `Compact` mode they `padding(innerPadding).consumeWindowInsets(innerPadding)` (bottomBar covers the bottom system inset); in `Rail` and `None` modes they consume only the top inset, leaving the bottom inset to flow downstream. Every screen-level `LazyColumn` reads the remaining `WindowInsets.safeDrawing` via the `safeListContentPadding()` helper in `Spacing.kt`, which combines the existing 8/24dp visual chrome clearance with `WindowInsets.safeDrawing.asPaddingValues()`. Result: in Wide / Expanded modes (no bottomBar) the last item can scroll past the gesture nav into the safe area, mid-scroll items draw under the gesture nav for the proper edge-to-edge look; in Compact mode the helper reduces to the legacy 8/24dp values because `safeDrawing.bottom` was consumed upstream. **The contract is: outer layers consume what they structurally handle; bottom inset flows to scrollable leaves; leaves all read `WindowInsets.safeDrawing` via `safeListContentPadding()`.** Adding new chrome (snackbar slot, side panel) only requires that chrome to declare its consumption upstream — leaves are unchanged. Landed in PR #759 (`android/226`, 2.16.12). **Don't** revert any leaf to `Spacing.ListContentPadding` in screen-level scrollables — that's the legacy fixed value that won't track future inset changes; reserve it for scrollables NOT at the body's bottom edge (e.g. `DiagnosticsContent`'s LazyColumn that has an action row below it).
