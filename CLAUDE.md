...
> The Gradle wrapper lives at `AndroidGarage/gradlew`, and `cd` is hook-blocked. From the repo root (where you run everything) invoke Gradle as **`AndroidGarage/gradlew -p AndroidGarage <task>`** — there is no root `./gradlew` (the `./gradlew` shown below assumes the `AndroidGarage/` working dir). Most work goes through `scripts/validate.sh` / `scripts/generate-ios-screenshots.sh`, which wrap this for you.
```bash
# Debug builds
...
**The complement (ADR-033, hard rule + enforced): UI-triggered work goes through the ViewModel — UI code must NOT reach a UseCase/Repository directly off the DI `component`.** Together these are total: UI-triggered → ViewModel; screen-less → UseCase-direct; nothing in between. A UseCase is exposed as a component entry point (`abstract val xUseCase`) ONLY for (a) a UI-less caller (FCM/`AppStartup`) or (b) `@Singleton`-DI machinery (kotlin-inject needs the `abstract val` to honor the scope — e.g. `computeButtonHealthDisplayUseCase`); anything else there is a smell. Platform side-effects that can't live in the shared KMP VM (clipboard write, file write, file-picker, share sheet) stay in the UI: the **VM owns the produce/decision** (fetch the token, build the CSV) and hands the value back; the **UI does only the irreducible platform write** — so the UI's *entry point* is a VM method, not a UseCase. Enforced by `checkUiLayerNoGraphAccess` (`buildSrc/.../architecture/UiLayerNoGraphAccessTask.kt`, in `validate.sh`): scans `androidApp/.../ui/` for the member-access pattern `component.<x>UseCase`/`component.<x>Repository` (member-access, NOT imports — a `:usecase` *data class* like `ButtonHealthDisplay` is legitimately imported by UI). `AndroidGarage/ui-layer-graph-access-exemptions.txt` grandfathers known pre-rule violations during burn-down (fails on a stale entry; goal = empty). iOS (unlinted by buildSrc) follows the same rule by convention + review. **Burn-down in progress:** copy-auth-token + Export CSV are being routed through their VMs; until then their two files are exempted.
...
**SHIPPED + verified end-to-end (`ios/1` = 0.1.0 uploaded to TestFlight, 2026-06-30).** `scripts/release-ios.sh` + `.github/workflows/release-ios.yml` mirror the Android release model. Flow: `./scripts/validate-ios.sh` (writes `.claude/.ios-validation-passed`) → `./scripts/release-ios.sh --check` → `--confirm-tag ios/N`. The tag `ios/N` sets `CFBundleVersion = N`; `MARKETING_VERSION` (X.Y.Z, from `project.yml`) drives the `AndroidGarage/iosApp/CHANGELOG.md` gate. Uploads to **TestFlight Internal only**.
