...
- **App Store Connect silently renumbers a duplicate build** instead of failing (that's how `ios/1` landed as build 2 after a manual upload took build 1). The CI pre-flight aborts on a collision with the exact number to use. Tag N and the build number can drift when a build is uploaded out-of-band; `--confirm-tag` accepts any strictly-higher `ios/N` (skip-ahead) so you can push the number CI names. **The next release is `ios/4`** (builds 1 + 2 are on App Store Connect; `ios/3`'s upload failed on the iOS-26-SDK gate below — archive + cloud-sign succeeded, only the upload was rejected — so build 3 went unused and the `ios/3` tag is spent).
