...
- **CI signs from a STORED cert, not cloud signing (since ios/12, 2026-08-01).** `-allowProvisioningUpdates` mints an Apple Distribution certificate on demand, and Apple caps those at a small number per account (commonly two; the portal is the authority) — so a job that creates one per release eventually fails every release at archive with `Your account has reached the maximum number of certificates` (plus a knock-on `No profiles for 'com.chriscartland.garage' were found`, which is Xcode falling back to Development signing). **Nothing reaches App Store Connect when this fires** — export/upload are skipped, the build number stays free, and the SAME tag can be re-run once signing is fixed; do not cut a new one. The cert + profile now live in `IOS_DIST_CERT_P12_BASE64` / `IOS_DIST_CERT_PASSWORD` / `IOS_PROVISIONING_PROFILE_BASE64` and are imported into a throwaway keychain. A cert minted by cloud signing **cannot be exported later** — its private key died with the runner — so recovering from the cap means revoking one and generating a fresh cert locally. Full runbook incl. expiry: [`docs/IOS_RELEASE_SETUP.md`](docs/IOS_RELEASE_SETUP.md) § "Signing: stored cert". The ASC API key is still required (the export step uploads with it); its Admin role was needed for cloud signing and is left in place — an App Manager key archived fine but failed export with `Cloud signing permission error` (empirical: `ios/1` attempt 1).
...
**How each platform stays fresh is a swappable strategy, not a platform fork.** `DoorUpdateStrategy` (`:usecase`) has three implementations — `PUSH` (no client timer; Android's always-shipped behavior), `POLL` (15s while visible + refresh on foreground; **iOS's default today**), and `PUSH_WITH_FOREGROUND_REFRESH` (the iOS destination once APNs delivery works). `DoorUpdateManager` (ADR-015) resolves `AppSettingsRepository.doorUpdateStrategy` against `AppConfig.defaultDoorUpdateStrategy` and runs exactly one, swapping **live** via `collectLatest` — a new value cancels the running strategy's coroutine and structured concurrency takes its timer, in-flight fetch, and visibility collector with it. UI gate on both platforms: Settings → Developer → "Door updates". Full design + rollout: [`docs/DOOR_UPDATE_STRATEGY.md`](docs/DOOR_UPDATE_STRATEGY.md).

- **Why iOS was dark:** `EventFCM.ts` sets `data` + `topic` + `android` and nothing else — `apns: ApnsConfig` is still commented out in `FirebaseServer/src/model/FCM.ts`. A data-only FCM message reaches an Apple device only with `apns.payload.aps.content-available = 1`. The whole iOS receive path (`AppDelegate.didReceiveRemoteNotification` → shared `FcmPayloadParser` → `ReceiveFcmDoorEventUseCase`) has been built and correct since #915; the server simply never addressed a message to it. **Fixing that is Phase 2 and wants its own PR + `server/N`** — it is on the path that carries every Android device's door updates, so the blast radius of getting it wrong is "Android goes dark too."
